        ...
        for input_seq_len in INPUT_SEQ_LENS:
            for output_seq_len in OUTPUT_SEQ_LENS:
                for max_concurrency in MAX_CONCURRENCIES:
                    ...
                    if output_seq_len == 1:
                        self.assertGreater(res["input_throughput"], EXPECTED_PERFORMANCE[max_concurrency][input_seq_len]["input_throughput"]*(1-ALLOW_GAP))
                        self.assertLess(res["median_ttft_ms"], EXPECTED_PERFORMANCE[max_concurrency][input_seq_len]["ttft"]*(1+ALLOW_GAP))
                    else:
                        self.assertGreater(res["output_throughput"],EXPECTED_PERFORMANCE[max_concurrency][input_seq_len]["output_throughput"]*(1-ALLOW_GAP))
                        self.assertLess(res["median_itl_ms"], EXPECTED_PERFORMANCE[max_concurrency][input_seq_len]["itl"]*(1+ALLOW_GAP))
