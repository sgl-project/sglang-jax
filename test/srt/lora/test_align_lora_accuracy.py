...
class TestAlignLoRAAccuracy(CustomTestCase):
    ...
    def setUpClass(cls):
        ...
        cls.single_prompt_prefill_logits_cpu_dump_filename = f"./lora_data/sglangjax/single_prompt_prefill_logits_cpu.txt"
        cls.single_prompt_decode_logits_cpu_dump_filename = f"./lora_data/sglangjax/single_prompt_decode_logits_cpu.txt"
        cls.multi_prompts_prefill_logits_tpu_dump_filename = f"./lora_data/sglangjax/multi_prompts_prefill_logits_tpu.txt"
        cls.multi_prompts_decode_logits_tpu_dump_filename = f"./lora_data/sglangjax/multi_prompts_decode_logits_tpu.txt"
    ...
    def get_sglang_jax_last_layer_logits_hidden_states(
        ...
        if return_logits:
            os.environ["DUMP_LAST_LAYER_LOGITS_FILENAMES"] = f"{prefill_return_logits_dump_filename},{decode_return_logits_dump_filename}"
        if device == "cpu":
        ...
        def get_content_from_files(
            ...
            if not os.path.exists(prefill_dump_filename) or not os.path.exists(decode_dump_filename):
                raise ValueError(f"{prefill_dump_filename} or {decode_dump_filename} does not exist!")
            ...
            if decode_dump_filename:
                while True:
                    ...
                    with  open(decode_dump_filename, 'r') as decode_f:
                        ...
                        decode_reshaped_data = np.array(decode_data,dtype=NPDTYPE).reshape(len(prompts),-1)
...
--dtype float32
"""
        hf_lora_prefill_logits_file = os.path.join(os.path.dirname(__file__), "lora_data/hf/single_prompt_prefill_output_cpu.json")
        hf_lora_decode_logits_file = os.path.join(os.path.dirname(__file__), "lora_data/hf/single_prompt_decode_output_cpu.json")
        
        try:
            assert os.path.exists(hf_lora_prefill_logits_file) and os.path.exists(hf_lora_decode_logits_file)
        except BaseException as e:
            print(f"{hf_lora_prefill_logits_file} or {hf_lora_decode_logits_file} does not exist, please generate them firstly with following commands: \n{hf_generate_commands}",flush=True)
            raise
        ...
        try:
            with open(hf_lora_prefill_logits_file, "r") as prefill_f, open(hf_lora_decode_logits_file, "r") as decode_f:
                hf_prefill_data = json.load(prefill_f)
                hf_decode_data = json.load(decode_f)
                hf_prefill_logits = np.array(hf_prefill_data["results"][0]["last_token_logits"],dtype=NPDTYPE).reshape(1,-1)
                hf_decode_logits = np.array(hf_decode_data["results"][0]["last_token_logits"], dtype=NPDTYPE).reshape(1,-1)
        except Exception as e:
            raise ValueError(f"Fail to load {hf_lora_prefill_logits_file} and {hf_lora_decode_logits_file} and meet err: {e}")

        sgl_prefill_logits, sgl_decode_logits,_,_=self.get_sglang_jax_last_layer_logits_hidden_states(
            [PROMPTS[0]], 
            LORA_PATHS[0],
        ...
        tolerance = 1e-4
        np.testing.assert_allclose(hf_prefill_logits, sgl_prefill_logits,atol=tolerance, rtol=tolerance)
        np.testing.assert_allclose(hf_decode_logits, sgl_decode_logits,atol=tolerance, rtol=tolerance)
...
"""
        ...
        hf_lora_decode_logits_file = os.path.join(os.path.dirname(__file__), "lora_data/hf/multi_prompts_decode_output_cpu.json")
        ...
        try:
            ...
            print(f"{hf_lora_decode_logits_file} does not exist, please generate it firstly with following commands: \n{hf_generate_commands}",flush=True)
            raise
        ...
        try:
            with open(hf_lora_decode_logits_file, "r") as decode_f:
                ...
                hf_decode_logits_multi_prompts = np.array(hf_decode_logits_list,dtype=NPDTYPE).reshape(len(PROMPTS),-1)
        except Exception as e:
            raise ValueError(f"Fail to {hf_lora_decode_logits_file} and meet err: {e}")

        _, sgl_decode_logits_multi_prompts,_,_=self.get_sglang_jax_last_layer_logits_hidden_states(
            PROMPTS, 
            LORA_PATHS, 
...
"""
        ...
        hf_lora_decode_logits_file = os.path.join(os.path.dirname(__file__), "lora_data/hf/multi_prompts_decode_output_cpu_1_layer.json")
        ...
        """Compare logprobs from sglang-jax with HuggingFace reference."""
        print("=================== testing multi prompts hidden states comparison ======================")
        ...
        try:
            with open(hf_lora_decode_logits_file, "r") as decode_f:
                ...
                for i in range(len(PROMPTS)):
                    hf_decode_hidden_states_list.append(hf_decode_data["results"][i]["last_layer_hidden_states"][-1])
                hf_decode_hidden_states_multi_prompts = np.array(hf_decode_hidden_states_list,dtype=NPDTYPE).reshape(len(PROMPTS),-1)
        except Exception as e:
            raise ValueError(f"Fail to {hf_lora_decode_logits_file} and meet err: {e}")

        _,_,_,sgl_decode_hidden_states_multi_prompts=self.get_sglang_jax_last_layer_logits_hidden_states(
            PROMPTS, 
            LORA_PATHS, 
            return_logits=False,
            device="tpu",
        ...
        for i in range(len(PROMPTS)):
            ...
            print(f"{hf_decode_hidden_states.shape=}, {hf_decode_hidden_states[...,:20]}",flush=True)
            #print(f"{sgl_prefill_logits.shape=}, {sgl_prefill_logits[...,:20]}",flush=True)
            print(f"{sgl_decode_hidden_states.shape=}, {sgl_decode_hidden_states[...,:20]}",flush=True)
            ...
            tolerance = 9e-2
            np.testing.assert_allclose(hf_decode_hidden_states, sgl_decode_hidden_states,atol=tolerance, rtol=tolerance)
