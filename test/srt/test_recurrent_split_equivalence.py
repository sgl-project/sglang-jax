...
forward over the whole prompt, the GDN recurrence is computed as several
...
(``ragged_gated_delta_rule_ref``). Both live in
``sgl_jax.srt.kernels.gdn.gated_delta`` and are pure JAX (eager on one CPU
device — no mesh needed). The pool-table dtypes model the production
round-trip: the kernels cast on write-back and ragged reads the recurrent
table back as float32, so the bf16/fp32 carry is controlled purely by the
dtype of the initial tables we allocate (production = conv pool bfloat16,
recurrent pool float32).
"""
