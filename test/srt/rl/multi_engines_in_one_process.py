...
class TestMultiEnginesInOneProcess(CustomTestCase):
    ...
    def test_02_multi_engine_modes(self):
        ...
        # try:
        #    print(f"=====================================generate in mode 1====================================", flush=True)
        #    outputs = self._run_generate(engine)
        ...
        # ------------------------------------------------------------------ #
        print("=== Mode 2: two engines, engineA=[0,1], engineB=[2,3] ===", flush=True)
        engine_a = _make_engine(device_indexes=[0, 1])
        print(
            f"################################################engineA is initialized#########################################",
            flush=True,
        )
        engine_b = _make_engine(device_indexes=[2, 3])
        print(
            f"################################################engineB is initialized#########################################",
            flush=True,
        )
        try:
            [print(f"\n") for _ in range(10)]
            print(
                f"=====================================generate in mode 2====================================",
                flush=True,
            )
            outputs = self._run_generate(engine_a)
