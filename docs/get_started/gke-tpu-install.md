...
First, set up your GKE cluster and provision a dedicated Cloud TPU node pool.
...
## Step 2: Create and Deploy the Kubernetes Manifest

Save the following manifest as `sglang-tpu-deployment.yaml`. This manifest uses the official public image `lmsysorg/sglang-jax:latest` and includes critical JAX/TPU optimizations explained in the **Friction Points & Key Insights** section below.

**NOTE**: The example YAML below is pre-configured to use the public image. You may still need to update other sections based on your specific TPU machine-type and node pool capacity. Look for the "NOTE:" comments in the YAML below and update as needed.
  ...
  template:
    ...
    spec:
      ...
      - name: sglang-container
        # NOTE: Uses the official public sglang-jax image
        image: lmsysorg/sglang-jax:latest
...
*   **The Fix**: As an example for the machine type used throughout the document here, to utilize all 8 chips of `ct5lp-hightpu-8t` for Qwen 7B, configure `--tp-size 8 --dp-size 2`. This correctly yields `dp_size = 2` and `actual_tp_size = 4` (which divides 28 perfectly).
