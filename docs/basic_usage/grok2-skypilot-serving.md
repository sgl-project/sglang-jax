...
--random-seed=3 --mem-fraction-static=0.8 --chunked-prefill-size=32 \
--download-dir=/dev/shm --dtype=bfloat16 --max-running-requests=128 \
--skip-server-warmup --page-size=32 --tokenizer-path alvarobartt/grok-2-tokenizer \
--disable-precompile --dist-init-addr=<NODE_0_IP_ADDRESS>:<PORT> \
