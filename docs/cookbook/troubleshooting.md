...
**Fix**: Use `--moe-backend epmoe` on single-host v7x-8 and small slices; switch to `--moe-backend fused` for multi-host setups. MiMo-V2-Flash recipe has measured numbers — see [its §6.2](autoregressive/mimo-v2-flash.md#62-throughput--bench_serving).
...
**Fix**: Pass `--tokenizer-path <community-tokenizer-repo>` explicitly. Example: Grok-2 uses `alvarobartt/grok-2-tokenizer` — see [`autoregressive/grok2.md`](autoregressive/grok2.md).
