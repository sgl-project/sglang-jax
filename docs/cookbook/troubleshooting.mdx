...
2. For v7x, remember each chip exposes 2 JAX devices — see [`base/tpu-topology-reference.mdx`](base/tpu-topology-reference.mdx).
...
**Fix**:
- Confirm your model is in the supported list — see the cookbook [autoregressive index](autoregressive/index.mdx) or [`python/sgl_jax/srt/models/`](https://github.com/sgl-project/sglang-jax/tree/main/python/sgl_jax/srt/models).
...
**Cause**: Sliding-window-attention pool is too small for the concurrent decode load. The `--swa-full-tokens-ratio` flag controls `swa_tokens_per_layer / full_tokens_per_layer` — see [`base/launch-flags-reference.mdx`](base/launch-flags-reference.mdx) for the per-layer semantics (it is **not** a pool fraction).
...
**Fix**: Use `--moe-backend epmoe` on single-host v7x-8 and small slices; switch to `--moe-backend fused` for multi-host setups. MiMo-V2-Flash recipe has measured numbers — see [its §6.2](autoregressive/Xiaomi/MiMo-V2-Flash.mdx#62-throughput--bench_serving).
...
**Fix**: Pass `--tokenizer-path <community-tokenizer-repo>` explicitly. Example: Grok-2 uses `alvarobartt/grok-2-tokenizer` — see [`autoregressive/grok2.mdx`](autoregressive/Grok/Grok2.mdx).
