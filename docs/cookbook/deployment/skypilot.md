...
Generic SkyPilot recipe for provisioning a multi-node TPU cluster and launching `sgl_jax.launch_server` on it. Cookbook recipes that use SkyPilot ([Grok-2](../autoregressive/grok2.md), and any future multi-node dense / MoE recipe) reference this page instead of duplicating the workflow.
...
|---|---|---|---|
| [Grok-2](../autoregressive/grok2.md) | `tpu-v6e-32` | 8 | 32 |

Single-host recipes ([Qwen-7B-Chat](../autoregressive/qwen.md), [Qwen3](../autoregressive/qwen3.md)) can also run via SkyPilot by passing `tpu-v6e-4`, but Docker on a single host is usually simpler — see the recipes themselves for the direct launch form.
