...
Generic SkyPilot recipe for provisioning a multi-node TPU cluster and launching `sgl_jax.launch_server` on it. Cookbook recipes that use SkyPilot ([Grok-2](../autoregressive/Grok/Grok2.mdx), and any future multi-node dense / MoE recipe) reference this page instead of duplicating the workflow.
...
For the broader SkyPilot development workflow (clone, sync, destroy) see [`../../developer_guide/tpu_resources_guide.mdx`](../../developer_guide/tpu_resources_guide.md).
...
| `<N>` | `--tp-size` = total JAX devices across all nodes. See [`../base/tpu-topology-reference.mdx`](../base/tpu-topology-reference.mdx). |
...
|---|---|---|---|
| [Grok-2](../autoregressive/Grok/Grok2.mdx) | `tpu-v6e-32` | 8 | 32 |

Single-host recipes ([Qwen-7B-Chat](../autoregressive/Qwen/Qwen.mdx), [Qwen3](../autoregressive/Qwen/Qwen3.mdx)) can also run via SkyPilot by passing `tpu-v6e-4`, but Docker on a single host is usually simpler — see the recipes themselves for the direct launch form.

## Related docs

- [`../../developer_guide/tpu_resources_guide.mdx`](../../developer_guide/tpu_resources_guide.md) — broader SkyPilot dev workflow (sync code, destroy clusters).
- [`../base/tpu-topology-reference.mdx`](../base/tpu-topology-reference.mdx) — TPU/device/chip table for picking `--tp-size`.
- [`../base/launch-flags-reference.mdx`](../base/launch-flags-reference.mdx) — full launch flag reference.
