...
[**Wan-AI/Wan2.2-T2V-A14B-Diffusers**](https://huggingface.co/Wan-AI/Wan2.2-T2V-A14B-Diffusers) is Alibaba's Wan 2.2 text-to-video diffusion checkpoint. It generates short videos from a text prompt by running a UMT5 text encoder, dual Wan DiT denoisers, and a 3D VAE decoder. SGL-JAX serves it through the multimodal pipeline (`--multimodal`), exposing the `POST /api/v1/videos/generation` endpoint.

**Variant**:

- [**Wan-AI/Wan2.2-T2V-A14B-Diffusers**](https://huggingface.co/Wan-AI/Wan2.2-T2V-A14B-Diffusers) - A14B dual-transformer checkpoint; current starter path runs on one v6e-4 host with `--tp-size 1`.

**Architectural notes**:

- **Three-stage SGL-JAX pipeline** - Wan 2.2 runs as a text encoder stage, a diffusion stage, and a VAE stage. The current SGL-JAX path places the text encoder on CPU and uses a fixed TPU split for video stages.
- **Dual-transformer denoising** - Wan 2.2 loads high-noise and low-noise transformers. The high-noise transformer handles early/coarse denoising steps, and the low-noise transformer handles late/fine steps. Both load at startup, so plan capacity from total resident weights, not just active denoising steps.
...
- Wan 2.2 uses the built-in staged runtime path associated with `--tp-size 1` on v6e-4.
- Do not scale Wan 2.2 by changing only `--tp-size`. Larger TPU slices require SGL-JAX support for a matching staged runtime path.
- Moving to v6e-8 does not automatically help unless SGL-JAX provides a matching larger-stage placement for this path.
...
- The current Wan 2.2 path runs text encoding on CPU. Changing text-encoder precision is therefore not a meaningful way to free TPU HBM for this recipe.
...
Video generation quality is typically evaluated subjectively (FVD, motion smoothness, prompt fidelity) rather than via a numeric `evalscope` dataset. There is no canonical automated accuracy benchmark for Wan 2.2 in this cookbook today; see the model card for reference quality numbers.
