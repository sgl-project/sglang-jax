...
Standard OpenAI-compatible request — see [`Qwen3.mdx` §3.1](../Qwen/Qwen3.mdx#31-basic-chat-completion) for the curl / Python pattern. Substitute `model="google/gemma-2-9b-it"` (or your chosen variant).

> Gemma 2 does not ship with native hybrid reasoning or a built-in tool-call format. For reasoning / tool-call workloads use a model with `--reasoning-parser` / `--tool-call-parser` support (see [`Qwen3.mdx` §3.2 / §3.3](../Qwen/Qwen3.mdx) or [`MiMo-V2.5-Pro.mdx`](../Xiaomi/MiMo-V2.5-Pro.mdx)).
...
**Benchmark Command** — adapt the driver from [`Qwen3.mdx` §4.2](../Qwen/Qwen3.mdx#42-speed--sgl-jax-vs-vllm) (swap `MODEL_NAME` to the Gemma checkpoint).
...
| Sliding-window pool exhaustion at high concurrency | `--swa-full-tokens-ratio` mismatches workload sequence-length distribution | Lower the ratio to give the sliding pool more capacity. See [troubleshooting §SWA pool exhaustion](../../troubleshooting.mdx#swa-pool-exhaustion-mimo-hybrid-attention-models). |
...
- [`../troubleshooting.mdx`](../../troubleshooting.mdx) — cross-recipe generic issues including the SWA pool exhaustion note.
