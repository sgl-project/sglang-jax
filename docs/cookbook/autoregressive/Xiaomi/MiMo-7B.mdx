...
For the larger Xiaomi MoE models, see [`MiMo-V2-Flash.mdx`](MiMo-V2-Flash.mdx) and [`MiMo-V2.5-Pro.mdx`](MiMo-V2.5-Pro.mdx) — these are different architectures (256-expert MoE with hybrid attention), not just larger MiMo-7B variants.
...
**Tool Calling:**
- MiMo-7B shares the `mimo` tool-call parser format with MiMo-V2.5-Pro. Add `--tool-call-parser mimo` when using the OpenAI tools API. See [`MiMo-V2.5-Pro.mdx` §3.3](MiMo-V2.5-Pro.mdx#33-tool-calling) for the request/response pattern.
...
Standard OpenAI-compatible request — see [`MiMo-V2.5-Pro.mdx` §3.1](MiMo-V2.5-Pro.mdx#31-basic-chat-completion) for the curl / Python pattern. Substitute `model="XiaomiMiMo/MiMo-7B-RL"` (or your chosen variant).

### 3.2 Reasoning / Tool Calling

MiMo-7B shares the `mimo` reasoning and tool-call parser formats with MiMo-V2.5-Pro — the full streaming Python client + multi-turn `Handling Tool Call Results` pattern in [`MiMo-V2.5-Pro.mdx` §3.2](MiMo-V2.5-Pro.mdx#32-reasoning-thinking-on-default-thinking-off-optional) and [§3.3](MiMo-V2.5-Pro.mdx#33-tool-calling) applies directly. Substitute the model path and the §2.3 launch flags above.
...
**Benchmark Command** — adapt the driver from [`Qwen3.mdx` §4.2](../Qwen/Qwen3.mdx#42-speed--sgl-jax-vs-vllm) (swap `MODEL_NAME` to `XiaomiMiMo/MiMo-7B-RL`).
...
- [`MiMo-V2-Flash.mdx`](MiMo-V2-Flash.mdx) and [`MiMo-V2.5-Pro.mdx`](MiMo-V2.5-Pro.mdx) — larger Xiaomi MoE models (different architecture).
