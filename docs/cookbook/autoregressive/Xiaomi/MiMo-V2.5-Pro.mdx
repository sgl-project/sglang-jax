...
Adapt [`../deployment/skypilot.mdx`](../../deployment/skypilot.mdx) with `tpu-v7x-16` or `tpu-v6e-64` accelerator. `${NODE_RANK}` comes from `${SKYPILOT_NODE_RANK}`.
...
- For smaller single-host MoE setups (EP ≤ 8), `epmoe` actually wins; see the measured tradeoff in [`MiMo-V2-Flash.mdx` §4.2](MiMo-V2-Flash.mdx#42-speed--single-workload-configuration-sweep).
...
- [`MiMo-V2-Flash.mdx`](MiMo-V2-Flash.mdx) — smaller sibling, same architectural family, has measured MoE backend comparison.
...
- [`../deployment/gke-indexed-job.mdx`](../../deployment/gke-indexed-job.mdx) / [`../deployment/skypilot.mdx`](../../deployment/skypilot.mdx)
