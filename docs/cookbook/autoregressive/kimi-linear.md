...
For GKE, adapt the manifest pattern from [`mimo-v2.5-pro.md` §2.3 Multi-host](mimo-v2.5-pro.md#23-launch) with `<JOB>=kimi-linear`, `<ACCELERATOR>=tpu-v6e-slice`, `<TOPOLOGY>=4x4`, `parallelism: 4` / `completions: 4`, and the launch flags above.
...
Standard OpenAI-compatible request — see [`qwen3.md` §3.1](qwen3.md#31-basic-chat-completion). Substitute `model="moonshotai/Kimi-Linear-48B-A3B-Instruct"`.

> Kimi-Linear-Instruct does not ship with a built-in tool-call format. For tool-call workloads use a model with `--tool-call-parser` support (see [`qwen3.md` §3.3](qwen3.md#33-tool-calling) or [`mimo-v2.5-pro.md` §3.3](mimo-v2.5-pro.md#33-tool-calling)).
...
**Benchmark Command** — adapt the driver from [`qwen3.md` §4.2](qwen3.md#42-speed--sgl-jax-vs-vllm) (swap `MODEL_NAME` to `moonshotai/Kimi-Linear-48B-A3B-Instruct`, remove the vLLM half).
