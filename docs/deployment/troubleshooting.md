...
Common failure modes across SGL-JAX serving paths, with the fix and what to grep in logs to confirm. Model-specific constraints live in each cookbook recipe's Deployment / Configuration Tips section; this page covers cross-cutting startup, multi-node, and runtime issues.
...
2. For v7x, remember each chip exposes 2 JAX devices.
...
**Fix**:
- Confirm your model is in the supported list in the cookbook model indexes or in [`python/sgl_jax/srt/models/`](https://github.com/sgl-project/sglang-jax/tree/main/python/sgl_jax/srt/models).
...
**Cause**: Sliding-window-attention pool is too small for the concurrent decode load. The `--swa-full-tokens-ratio` flag controls `swa_tokens_per_layer / full_tokens_per_layer`; it is **not** a pool fraction.
...
**Fix**: Use `--moe-backend epmoe` on single-host v7x-8 and small slices; switch to `--moe-backend fused` for multi-host setups. Keep measured model-specific numbers in the relevant cookbook recipe.
...
**Fix**: Pass `--tokenizer-path <community-tokenizer-repo>` explicitly. Example: Grok-2 uses `alvarobartt/grok-2-tokenizer`.
