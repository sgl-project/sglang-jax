...
ROUTER_TEST_EXTERNAL_DP_ROUTING="${ROUTER_TEST_EXTERNAL_DP_ROUTING:-0}"
...
RUN_MMLU_PRO="${RUN_MMLU_PRO:-0}"
MMLU_PRO_Q="${MMLU_PRO_Q:-200}"
MMLU_PRO_PAR="${MMLU_PRO_PAR:-128}"
MMLU_PRO_MAXTOK="${MMLU_PRO_MAXTOK:-2048}"
MMLU_PRO_ENABLE_THINKING="${MMLU_PRO_ENABLE_THINKING:-1}"
MMLU_PRO_TIMEOUT_SECONDS="${MMLU_PRO_TIMEOUT_SECONDS:-7200}"
...
render() {
  sed \
    ...
    -e "s|<ROUTER_TEST_EXTERNAL_DP_ROUTING>|$(escape_sed "$ROUTER_TEST_EXTERNAL_DP_ROUTING")|g" \
    ...
    -e "s|<ENABLE_REQUEST_TIME_STATS_LOGGING>|$(escape_sed "$ENABLE_REQUEST_TIME_STATS_LOGGING")|g" \
    ...
    -e "s|<RUN_MMLU_PRO>|$(escape_sed "$RUN_MMLU_PRO")|g" \
    -e "s|<MMLU_PRO_Q>|$(escape_sed "$MMLU_PRO_Q")|g" \
    -e "s|<MMLU_PRO_PAR>|$(escape_sed "$MMLU_PRO_PAR")|g" \
    -e "s|<MMLU_PRO_MAXTOK>|$(escape_sed "$MMLU_PRO_MAXTOK")|g" \
    -e "s|<MMLU_PRO_ENABLE_THINKING>|$(escape_sed "$MMLU_PRO_ENABLE_THINKING")|g" \
    -e "s|<MMLU_PRO_TIMEOUT_SECONDS>|$(escape_sed "$MMLU_PRO_TIMEOUT_SECONDS")|g" \
