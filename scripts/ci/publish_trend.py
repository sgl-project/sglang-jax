...
def prepare_files_for_upload(source_dir, target_base_path):
    ...
    # Walk through source directory and find .png files
...
def publish_traces(traces_dir, run_id, run_number):
    ...
    print(f"Found {len(files_to_upload)} png files to upload")
    ...
    for attempt in range(max_retries):
        try:
            ...
            # Create commit
            commit_message = f"png files of Nightly-test for run {run_id} at {run_number} ({len(files_to_upload)} files)"
            ...
            print("Successfully published all png files in a single commit")
            return
