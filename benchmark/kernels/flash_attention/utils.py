...
def create_split_kv_cache_data(
    max_kv_cache_tokens, head_num, k_head_dim, v_head_dim, page_size=16, dtype=jnp.bfloat16, seed=42
):
    key = jax.random.PRNGKey(seed)
    keys = jax.random.split(key, 3)
    total_num_pages = cdiv(max_kv_cache_tokens, page_size)
    k_cache = jax.random.normal(
        keys[1], (total_num_pages, page_size, head_num, k_head_dim), dtype=dtype
    )
    v_cache = jax.random.normal(
        keys[2], (total_num_pages, page_size, head_num, v_head_dim), dtype=dtype
    )
...
def create_split_prefill_uniform_data(
    max_context_len,
    max_kv_cache_tokens,
    ...
    q, k, v = create_split_qkv_data(
        max_num_batched_tokens, q_head_num, kv_head_num, k_head_dim, v_head_dim, dtype, seed
    )
...
def create_split_decode_uniform_data(
    max_context_len,
    max_kv_cache_tokens,
    max_num_batched_tokens,
    q_head_num,
