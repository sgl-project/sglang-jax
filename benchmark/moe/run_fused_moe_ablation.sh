...
#   - A2A requires ALL_REDUCE_METADATA + SYNC_BARRIER.
# - For "one-enabled" stage profiling, this script uses a baseline where
#   ALL_REDUCE_METADATA + SYNC_BARRIER stay enabled ("control_only"),
...
build_env_all_disable() {
  # Disable everything (including ALL_REDUCE_METADATA / SYNC_BARRIER).
...
build_env_with_one_enabled() {
  ...
  case "${enabled_var}" in
    FUSED_MOE_BENCHMARK_DISABLE_ALL_REDUCE_METADATA|FUSED_MOE_BENCHMARK_DISABLE_SYNC_BARRIER)
...
echo ">>> [3/${TOTAL_CASES}] CONTROL ONLY (METADATA + BARRIER)"
for i in "${!COUNTS[@]}"; do
  c=${COUNTS[$i]}
  echo -n "  hotspot_count=$c ... "
