"""Profile FusedEPMoE bf16 vs fp8: capture xprof traces for comparison."""
...
def profile_scenario(num_tokens, trace_dir, quantize_to_fp8=False):
    """Profile a single scenario with xprof trace capture."""
    label = "fp8" if quantize_to_fp8 else "bf16"
    mesh = build_mesh(ep_size=EP_SIZE, tp_size=1)
    case = MoEBenchmarkCase(
        name=f"profile_{label}_nt{num_tokens}",
    ...
    with jax.set_mesh(mesh):
        ...
        if quantize_to_fp8:
            ...
            print(f"    Quantized to fp8. w1 dtype={fused_layer.w1[...].__class__.__name__}")
        ...
        print(
            f"    {label} nt={num_tokens}: median={median:.3f}ms  times={[f'{t:.3f}' for t in times]}"
        )

        # Capture xprof trace
        scenario_trace_dir = os.path.join(trace_dir, f"{label}_nt{num_tokens}")
        ...
        with jax.profiler.trace(scenario_trace_dir):
            for i in range(PROFILE_STEPS):
                with jax.profiler.StepTraceAnnotation(
                    f"fused_moe_{label}_nt{num_tokens}", step_num=i
                ):
                    out = fn(data["tokens"], data["router_logits"], **kwargs)
                    jax.block_until_ready(out)
...
def main():
    ...
    trace_dir = os.environ.get("PROFILE_DIR", "/gcs/prayer/moe_profiling/20260408_fp8_compare")

    print("=" * 80)
    print("FusedEPMoE bf16 vs fp8 Profiling (xprof trace)")
    print("=" * 80)
    ...
    # Profile both bf16 and fp8 at nt=4096 (worst regression) and nt=16384
    for nt in [4096, 16384]:
        print(f"\n--- num_tokens={nt} ---")
        profile_scenario(nt, trace_dir, quantize_to_fp8=False)
        profile_scenario(nt, trace_dir, quantize_to_fp8=True)
