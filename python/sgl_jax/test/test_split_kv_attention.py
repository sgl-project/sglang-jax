...
class TestSplitKernelAttention(CustomTestCase):
    ...
    def test_split_kernel_sliding_window_attention_sink_decode_ps16(self):
        """Decode with both sliding_window and attention_sink (MiMo-V2-Flash style)."""
        self.run_kernel_test(
            mode="decode",
            lens=[(1, 128), (1, 256)],
            num_heads=16,
            ...
            sliding_window=32,
            num_kv_pages_per_block=4,
            attention_sink=jnp.linspace(-0.4, 0.4, 16, dtype=jnp.float32),
        )
...
# ===================================================================
def test_flashattention_uses_split_path_for_wrapped_split_cache(monkeypatch):
    ...
    split_result = object()
    fused_result = object()
    monkeypatch.setattr(backend, "_call_split", lambda *args, **kwargs: split_result)
    monkeypatch.setattr(backend, "_call_fused", lambda *args, **kwargs: fused_result)

    q = jnp.zeros((1, 4, 128), dtype=jnp.bfloat16)
    token_to_kv_pool = SimpleNamespace(is_split=True)
    layer = SimpleNamespace(layer_id=0)

    result = backend(q, q, q, layer, None, token_to_kv_pool)
