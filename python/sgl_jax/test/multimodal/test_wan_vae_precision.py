...
class TestWan22VaePrecision(unittest.TestCase):
    """Test Wan2.2 VAE (is_residual=True) encode and decode precision"""
    ...
    def setUpClass(cls):
        ...
        # Use config registry to get the correct config with is_residual=True
        from sgl_jax.srt.multimodal.configs.config_registry import get_vae_config

        model_config = get_vae_config(cls.server_args.model_path)
        model_config.model_path = cls.server_args.model_path
        model_config.model_class = cls.model_class
    ...
    def _get_diffusers_encode_output(self):
        """Get diffusers encode output for Wan2.2 VAE.

        To generate reference data, run the following code with PyTorch:
        ```python
        import torch
        import numpy as np
        ...
        np.save("wan22_vae_diffusers_encode_output.npy", latents.latent_dist.parameters.detach().numpy())
        ```
        """
        current_dir = str(Path(__file__).resolve().parent)
        return np.load(current_dir + "/data/wan22_vae_diffusers_encode_output.npy")
    ...
    def _get_jax_encode_output(self):
        ...
        latents = self.jax_vae.encode(input)
        print(f"Wan2.2 JAX encode output shape: {latents.parameters.shape}")
        return latents.parameters.transpose((0, 4, 1, 2, 3))

    def _get_diffusers_decode_output(self):
        """Get diffusers decode output for Wan2.2 VAE.
        ...
        with torch.no_grad():
            y = vae.decode(latents)
        np.save("wan22_vae_diffusers_decode_output.npy", y.sample.detach().numpy())
        ```
        """
        current_dir = str(Path(__file__).resolve().parent)
        return np.load(current_dir + "/data/wan22_vae_diffusers_decode_output.npy")
    ...
    def _get_jax_decode_output(self):
        ...
        y = self.jax_vae.decode(latents)
        print(f"Wan2.2 JAX decode output shape: {y.shape}")
        return y.transpose((0, 4, 1, 2, 3))

    def test_encode_precision(self):
        print("Testing Wan2.2 encode precision...")
        torch_output = self._get_diffusers_encode_output()
        jax_output = self._get_jax_encode_output()
        np.testing.assert_allclose(torch_output, jax_output, rtol=1e-5, atol=1e-5)

    def test_decode_precision(self):
        print("Testing Wan2.2 decode precision...")
        torch_output = self._get_diffusers_decode_output()
        jax_output = self._get_jax_decode_output()
        np.testing.assert_allclose(torch_output, jax_output, rtol=1e-5, atol=1e-5)
