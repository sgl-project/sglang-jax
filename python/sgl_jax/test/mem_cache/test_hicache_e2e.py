...
class TestWriteThrough(HiCacheE2EBase):
    def test_backup_triggers_on_reuse_and_releases_lock(self):
        ...
        self.cache.insert(InsertParams(key=_key(tokens), value=idx, prev_prefix_len=len(idx)))
    ...
    def test_threshold_gt_one(self):
        ...
        node = self._child_of_root()
        self.cache.insert(
            InsertParams(key=_key(tokens), value=idx, prev_prefix_len=len(idx))
        )  # hit_count=1
        self.assertFalse(node.backuped)
        self.cache.insert(
            InsertParams(key=_key(tokens), value=idx, prev_prefix_len=len(idx))
    ...
    def test_load_back_respects_mem_quota(self):
        ...
        self.cache.insert(
            InsertParams(
                ...
                prev_prefix_len=len(np.concatenate([idx_a, idx_b])),
            )
        )
        self._settle_writes()
...
class TestTombstoneRevival(HiCacheE2EBase):
    ...
    def test_reinsert_with_extension_no_live_below_tombstone(self):
        ...
        node = self._child_of_root()
        self.cache.insert(
            InsertParams(key=_key(head), value=idx, prev_prefix_len=len(idx))
        )  # back up
        self._settle_writes()
    ...
    def test_lock_accounting_balanced_after_revive_and_reevict(self):
        ...
        self.cache.insert(InsertParams(key=_key(tokens), value=new_idx))
        self.cache.insert(
            InsertParams(key=_key(tokens), value=new_idx, prev_prefix_len=len(new_idx))
        )  # re-backup
        self._settle_writes()
        self.cache.evict(EvictParams(num_tokens=len(tokens), dp_rank=0))
...
class TestPartialHit(HiCacheE2EBase):
    ...
    def _run(self):
        ...
        self.cache.insert(InsertParams(key=_key(seg_a), value=idx_a, prev_prefix_len=len(idx_a)))
        self.cache.insert(
            InsertParams(key=_key(seg_a + seg_b), value=full, prev_prefix_len=len(full))
        )
        self._settle_writes()
...
class TestLoadBackPreEvicts(HiCacheE2EBase):
    ...
    def test_pre_eviction(self):
        ...
        self.cache.insert(
            InsertParams(key=_key(target), value=idx, prev_prefix_len=len(idx))
        )  # backup
        node = self._child_of_root()
        self._settle_writes()
...
class TestHiCacheEquivalence(unittest.TestCase):
    ...
    def test_match_depth_equivalent_under_eviction(self):
        ...
        self.cache_on.insert(
            InsertParams(key=_key(tokens), value=idx_on, prev_prefix_len=len(idx_on))
        )
        self._settle()
        ...
        self.cache_off.insert(
            InsertParams(key=_key(tokens), value=idx_off, prev_prefix_len=len(idx_off))
        )
