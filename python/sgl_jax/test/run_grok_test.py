...
def check_sglang_dependencies():
    ...
    try:
        ...
        print("✓ Grok1ForCausalLM available")
        return True
    except ImportError as e:
...
def run_tests(test_name=None, model_path=None, verbose=False):
    """Run the Grok MoE JAXModelLoader tests"""
    env = os.environ.copy()
    if model_path:
...
def create_sample_grok_moe_model(output_dir):
    """Create a sample Grok MoE JAX model directory for testing"""
    import json
    ...
    # Create config.json for Grok2 MoE model
    config = {
        "model_type": "grok",
...
def main():
    ...
    if args.create_sample:
        try:
            model_path = create_sample_grok_moe_model(args.create_sample)
    ...
    print("\nRunning Grok MoE JAXModelLoader tests...")
    ...
    if success:
        print("\n✓ All Grok MoE tests passed!")
        return 0
    else:
        print("\n✗ Some Grok MoE tests failed")
        return 1
