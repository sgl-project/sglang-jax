...
class TestMLAAttention(CustomTestCase):
    ...
    @unittest.skipIf(
        ...
        "Requires TPU: KV cache update and attention use Pallas kernels",
    )
    def test_radix_attention_integration(self):
        ...
        page_size = 1
        batch_size = 2
        seq_lens_list = [4, 6]  # KV lengths (prefix already cached)
        q_lens_list = [1, 1]  # decode: 1 new token per seq
        ...
        total_q_tokens = sum(q_lens_list)
        seq_lens = np.array(seq_lens_list, dtype=np.int32)
        aligned_seq_lens = ((seq_lens + page_size - 1) // page_size) * page_size
        total_aligned_tokens = int(np.sum(aligned_seq_lens))
        max_pool_size = total_aligned_tokens + 256

        # MLA pads V to qk_head_dim (192) inside __call__. MHATokenToKVPool
        # additionally requires head_dim aligned to 128, so 192 -> 256.
        token_to_kv_pool = MHATokenToKVPool(
            size=max_pool_size,
            page_size=page_size,
            dtype=jnp.bfloat16,
            head_num=NUM_HEADS,
            head_dim=(QK_HEAD_DIM + 127) // 128 * 128,
            layer_num=1,
            mesh=self.mesh,
        )
        ...
        # Build cache_loc (all tokens for all seqs, aligned)
        cache_loc_flat = []
        current_pos = 0
        for kv_len in seq_lens_list:
            aligned_len = ((kv_len + page_size - 1) // page_size) * page_size
            seq_indices = list(range(current_pos, current_pos + kv_len))
        ...
        # Decode mode: out_cache_loc = last token of each seq
        cache_start = np.concatenate([[0], np.cumsum(aligned_seq_lens)])
        out_cache_loc = np.array(
            [cache_loc[int(cache_start[i]) + seq_lens_list[i] - 1] for i in range(batch_size)],
            dtype=np.int32,
        )

        # Write prefix KV into pool (random data, we only check shapes/no-crash)
        rng = np.random.default_rng(42)
        for i in range(batch_size):
            prefix_len = seq_lens_list[i] - q_lens_list[i]
            if prefix_len > 0:
                start = int(cache_start[i])
                prefix_loc = jnp.array(cache_loc[start : start + prefix_len], dtype=jnp.int32)
                prefix_k = jnp.array(
                    rng.standard_normal((prefix_len, NUM_HEADS, QK_HEAD_DIM)).astype(np.float32),
                    dtype=jnp.bfloat16,
                )
                prefix_v = jnp.array(
                    np.pad(
                        rng.standard_normal((prefix_len, NUM_HEADS, V_HEAD_DIM)).astype(np.float32),
                        ((0, 0), (0, 0), (0, QK_HEAD_DIM - V_HEAD_DIM)),
                    ),
                    dtype=jnp.bfloat16,
                )
                token_to_kv_pool.set_kv_buffer(0, prefix_loc, prefix_k, prefix_v)

        input_ids = np.arange(total_q_tokens, dtype=np.int32)
        # Cache position indices for ForwardBatch metadata (not RoPE positions)
        cache_positions = np.arange(total_aligned_tokens, dtype=np.int32)
        ...
        mwb = ModelWorkerBatch(
            ...
            input_ids=input_ids,
            real_input_ids_len=total_q_tokens,
            seq_lens=seq_lens,
        ...
        fb = ForwardBatch(
            ...
            positions=jnp.array(cache_positions),
        ...
        # Run MLA end-to-end
        hidden = jnp.array(
            rng.standard_normal((total_q_tokens, HIDDEN_SIZE)).astype(np.float32),
            dtype=jnp.bfloat16,
        )
        pos = jnp.array([seq_lens_list[i] - 1 for i in range(batch_size)], dtype=jnp.int32)

        output, kv_fused = mla(pos, hidden, fb, token_to_kv_pool)

        output_np = np.array(output)
        self.assertEqual(output_np.shape, (total_q_tokens, HIDDEN_SIZE))
        self.assertFalse(np.any(np.isnan(output_np)), "Output contains NaN")
        self.assertFalse(np.any(np.isinf(output_np)), "Output contains Inf")
