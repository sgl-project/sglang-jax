...
class ScheduleBatch:
    ...
    def get_model_worker_batch(
        ...
        extend_prefix_lens = np.array(
            [] if extend_prefix_lens is None else extend_prefix_lens
        )
        extend_seq_lens = np.array([] if extend_seq_lens is None else extend_seq_lens)
        logging.info(
        ...
        data_sharding = NamedSharding(self.mesh, P(None))
        logging.info(
            f"schedule_batch.get_model_worker_batch: {self.forward_mode.name} input_ids_cpu = {input_ids_cpu} sum = {np.sum(input_ids_cpu)}"
        )
