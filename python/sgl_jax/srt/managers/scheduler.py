...
class Scheduler(
    ...
    def get_new_batch_prefill(self) -> Optional[ScheduleBatch]:
        ...
        if (
            ...
            if not self.running_batch.is_empty():
                ...
                # # Debug: Mixed mode info
                # print(f"[MIXED MODE] Mixing {len(new_batch.reqs)} prefill reqs + {len(self.running_batch.reqs)} decode reqs")
                # for i, req in enumerate(new_batch.reqs):
                #     print(f"  Prefill req {req.rid}: input_len={len(req.fill_ids)}")
                # for i, req in enumerate(self.running_batch.reqs):
                #     print(f"  Decode req {req.rid}: seq_len={len(req.origin_input_ids + req.output_ids)}, next_token_pos={len(req.origin_input_ids + req.output_ids)}")

                new_batch.mix_with_running(self.running_batch)
                new_batch.decoding_reqs = self.running_batch.reqs

                # print(f"[MIXED MODE] Final batch: extend_seq_lens={new_batch.extend_lens}")
    ...
    def run_batch(self, batch: ScheduleBatch) -> Union[GenerationBatchResult]:
        ...
        model_worker_batch = batch.get_model_worker_batch(
            ...
            precompile_prefill_cache_loc_paddings,
            precompile_decode_cache_loc_paddings,
            traced_req_indices=traced_req_indices,
        )
        if model_worker_batch.traced_token_indices is not None:
            print("=== Model worker batch info ===")
            print(f"real_bs: {model_worker_batch.real_bs}")
            print(f"forward_mode: {model_worker_batch.forward_mode}")
            print(f"input_ids: {model_worker_batch.input_ids}")
            print(f"real_input_ids_len: {model_worker_batch.real_input_ids_len}")
