# copy from https://github.com/sgl-project/sglang/blob/main/python/sglang/srt/tokenizer/tiktoken_tokenizer.py
import functools
import json
...
class TiktokenTokenizer:
    def __init__(self, tokenizer_path):
        ...
        special_tokens = {
            bytes(item["bytes"]).decode(): item["token"] for item in xtok_dict["special_tokens"]
        }
        if xtok_dict["word_split"] == "V1":
            pad_str = PAT_STR_B
        else:
            raise ValueError(f"Unknown word_split: {xtok_dict['word_split']}")
        ...
        if "default_allowed_special" in xtok_dict:
            default_allowed_special = set(
                [bytes(bytes_list).decode() for bytes_list in xtok_dict["default_allowed_special"]]
            )
        ...
        def encode_patched(
            ...
            text: str,
            *,
            allowed_special: Literal["all"] | Set[str] = set(),  # noqa: B006
