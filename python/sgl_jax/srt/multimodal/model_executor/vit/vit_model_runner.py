...
class VitModelRunner(BaseModelRunner):
    ...
    def forward(self, batch: Req, mesh: jax.sharding.Mesh):
        ...
        # assemble_mm_inputs no longer emits a single combined "pixel_values" (it was dropped when
        # the in-model path split it into per-modality keys); the ViT encode still wants ONE patch
        # tensor matching combined_grid_thw == image_grid_thw ++ video_grid_thw, so concat the
        # image patches then the video patches in that same order.
        pv_parts = [
            p
            for p in (assembled["pixel_values_images"], assembled["pixel_values_videos"])
            if p is not None
        ]
        pixel_values = np.concatenate(pv_parts, axis=0) if pv_parts else None
