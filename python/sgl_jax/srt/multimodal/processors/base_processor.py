...
class BaseMultiModalProcessorOutput:
    ...
    # videos
    videos: Optional[list[Union[jax.Array, dict]]] = dataclasses.field(
        default_factory=list
    )

    # audios
    audios: Optional[list[Union[jax.Array, dict]]] = dataclasses.field(
        default_factory=list
    )
...
class BaseMultimodalProcessor(ABC):
    ...
    def get_mm_items_offset(
        input_ids: jax.Array, mm_token_id: int
    ) -> List[Tuple[int, int]]:
        """
    ...
    ) -> Tuple[List[MultimodalDataItem], jax.Array, dict]:
        """
