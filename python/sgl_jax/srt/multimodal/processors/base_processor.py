...
from sgl_jax.srt.utils import is_npu, load_audio, load_image, load_video, logger
...
class BaseMultiModalProcessorOutput:
    ...
    images: list[Image.Image | dict] | None = dataclasses.field(default_factory=list)

    # videos
    videos: list[jnp.ndarray | dict] | None = dataclasses.field(default_factory=list)

    # audios
    audios: list[np.ndarray | dict] | None = dataclasses.field(default_factory=list)
...
class MultimodalSpecialTokens:
    ...
    image_token_regex: re.Pattern | None = None
    video_token_regex: re.Pattern | None = None
    audio_token_regex: re.Pattern | None = None

    combined_regex: re.Pattern | None = None
    ...
    def convert_to_str(self, token: str | int, processor) -> str:
        if token is None:
            return token
    ...
    def get_modality_of_token(self, token: str) -> Modality | None:
        """
...
class BaseMultimodalProcessor(ABC):
    ...
    def process_mm_data(self, input_text, images=None, videos=None, audios=None, **kwargs) -> dict:
        """
        ...
        if (
            ...
            if not _is_npu:
                device = jax.devices()[0]  # Use default JAX device
    ...
    def submit_data_loading_tasks(
        ...
        image_estimated_frames_iter: Iterator[int] | None = None,
            ...
            if modality is not None:
                ...
                if modality == Modality.IMAGE and image_estimated_frames_iter:
                    try:
                        ...
                        frame_count_limit = max(1, int(estimated_frames * image_scaling_factor))
            ...
            try:
                if multimodal_tokens_pattern.match(text_part):
                    ...
                    if modality == Modality.IMAGE:
                        ...
                        # token.
                        mm_tokens = text_part if is_precomputed else multimodal_tokens.image_token
                        ...
                        # load as video
                        mm_tokens = text_part if is_precomputed else multimodal_tokens.video_token
                        videos += [result]
                        new_text_parts += mm_tokens
                    elif modality == Modality.AUDIO:
                        # audio
                        mm_tokens = text_part if is_precomputed else multimodal_tokens.audio_token
                        audios += [result]
                        new_text_parts += mm_tokens
                else:
                    # normal text
    ...
    @staticmethod
    def get_mm_items_offset(input_ids: jnp.ndarray, mm_token_id: int) -> list[tuple[int, int]]:
        """
        ...
        # Handle first element separately to avoid roll boundary issue
        start_mask = mask.at[0].set(mask[0])
        start_mask = start_mask.at[1:].set(mask[1:] & ~mask[:-1])
        start_positions = jnp.where(start_mask)[0].tolist()

        # Handle last element separately
        end_mask = mask.at[-1].set(mask[-1])
        end_mask = end_mask.at[:-1].set(mask[:-1] & ~mask[1:])
        end_positions = jnp.where(end_mask)[0].tolist()

        return list(zip(start_positions, end_positions))

    @staticmethod
    def get_mm_items_offset_by_pair(
        input_ids: jnp.ndarray, mm_start_id: int, mm_end_id: int
    ) -> list[tuple[int, int]]:
        indices_start = jnp.where(input_ids == mm_start_id)[0] + 1
        indices_end = jnp.where(input_ids == mm_end_id)[0] - 1
    ...
    ) -> tuple[list[MultimodalDataItem], jnp.ndarray, dict]:
        """
