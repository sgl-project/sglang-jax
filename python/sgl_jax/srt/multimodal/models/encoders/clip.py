...
class CLIPAttention(nnx.Module):
    ...
    def __call__(self, x, attention_mask=None, deterministic=True):
        ...
        scores = jnp.matmul(q_h.astype(jnp.float32), jnp.transpose(k_h.astype(jnp.float32), (0, 1, 3, 2))) * self.scale
...
class CLIPMLP(nnx.Module):
    def __init__(self, config, mesh, dtype=jnp.bfloat16):
        ...
        self.fc1 = LinearBase(config.hidden_size, config.intermediate_size, mesh=mesh, use_bias=True, kernel_axes=(None, "tensor"), params_dtype=dtype)
        self.fc2 = LinearBase(config.intermediate_size, config.hidden_size, mesh=mesh, use_bias=True, kernel_axes=("tensor", None), params_dtype=dtype)
...
class CLIPEncoder(nnx.Module):
    def __init__(self, config, mesh, num_hidden_layers_override=None, dtype=jnp.bfloat16):
        num_layers = num_hidden_layers_override if num_hidden_layers_override is not None else config.num_hidden_layers
...
class CLIPVisionEmbeddings(nnx.Module):
    def __init__(self, config: CLIPVisionConfig, mesh, dtype=jnp.bfloat16):
        ...
        self.position_embedding = Embed(self.num_positions, self.embed_dim, dtype=dtype, param_dtype=dtype, mesh=mesh, kernel_axes=(None, "tensor"))
...
class CLIPVisionModel(nnx.Module):
    ...
    def _weight_mappings(self):
        m = {
            ...
            "vision_model.pre_layrnorm.bias": WeightMapping("pre_layrnorm.bias", (None,), False),
        }

        layer_count = len(self.encoder.layers)
        for i in range(layer_count):
            m.update(_layer_mappings(self.config, i, "vision_model.encoder.layers", "encoder.layers"))

        if self.post_layernorm is not None:
            m["vision_model.post_layernorm.weight"] = WeightMapping("post_layernorm.scale", (None,), False)
            m["vision_model.post_layernorm.bias"] = WeightMapping("post_layernorm.bias", (None,), False)

        return m

    def __call__(self, pixel_values: jax.Array, feature_sample_layers: Optional[List[int]] = None, output_hidden_states: Optional[bool] = None, deterministic=True):
        x = self.embeddings(pixel_values)
        x = self.pre_layrnorm(x)

        return_all = output_hidden_states or (feature_sample_layers is not None)
        encoder_outputs = self.encoder(x, return_all_hidden_states=return_all, deterministic=deterministic)
        ...
        # Handling feature sampling for VLM hooks
        if feature_sample_layers is not None:
            ...
            return selected_outputs[-1], selected_outputs

        return encoder_outputs[-1], encoder_outputs
...
class CLIPTextEmbeddings(nnx.Module):
    def __init__(self, config: CLIPTextConfig, mesh, dtype=jnp.bfloat16):
        self.token_embedding = Embed(config.vocab_size, config.hidden_size, dtype=dtype, param_dtype=dtype, mesh=mesh, kernel_axes=("tensor", None))
        self.position_embedding = Embed(config.max_position_embeddings, config.hidden_size, dtype=dtype, param_dtype=dtype, mesh=mesh, kernel_axes=(None, "tensor"))

    def __call__(self, input_ids: jax.Array, position_ids: Optional[jax.Array] = None):
...
class CLIPTextModel(nnx.Module):
    ...
    def _weight_mappings(self):
        m = {
            "text_model.embeddings.token_embedding.weight": WeightMapping("embeddings.token_embedding.embedding", ("tensor", None), False),
            "text_model.embeddings.position_embedding.weight": WeightMapping("embeddings.position_embedding.embedding", (None, "tensor"), False),
            "text_model.final_layer_norm.weight": WeightMapping("final_layer_norm.scale", (None,), False),
            "text_model.final_layer_norm.bias": WeightMapping("final_layer_norm.bias", (None,), False),
        }
