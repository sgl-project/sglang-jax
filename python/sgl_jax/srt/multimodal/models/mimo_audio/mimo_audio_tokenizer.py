...
def make_sequence_mask(lengths: Array, max_length: int | None = None) -> Array:
...
def get_position_ids(lengths: Array, max_length: int | None = None) -> Array:
...
class MiMoRotaryEmbedding(nnx.Module):
    ...
    def __init__(
        ...
        half_dim = dim // 2
        inv_freq = 1.0 / (
            self.base ** (jnp.arange(0, half_dim, dtype=jnp.float32) / float(half_dim))
        )
...
class ResidualVectorQuantizer(nnx.Module):
    ...
    def encode(
        self, hidden_states: Array, mask: Array | None = None, n_q: int | None = None
    ) -> tuple[Array, Array]:
...
class Attention(nnx.Module):
    ...
    def _window_mask(self, seq_len: int) -> Array | None:
    ...
    def __call__(self, x: Array, mask: Array | None, rope: tuple[Array, Array] | None) -> Array:
...
class TransformerLayer(nnx.Module):
    ...
    def __init__(
        ...
        self.self_attn_layer_norm = nnx.LayerNorm(
            d_model, epsilon=1e-6, param_dtype=dtype, rngs=rngs
        )
        self.final_layer_norm = nnx.LayerNorm(d_model, epsilon=1e-6, param_dtype=dtype, rngs=rngs)

        self.fc1 = LinearBase(
            d_model, ffn_dim, mesh=mesh, params_dtype=dtype, kernel_axes=(None, None)
        )
        self.fc2 = LinearBase(
            ffn_dim, d_model, mesh=mesh, params_dtype=dtype, kernel_axes=(None, None)
        )

    def __call__(
        self, hidden_states: Array, mask: Array | None, rope: tuple[Array, Array] | None
    ) -> Array:
        residual = hidden_states
...
class CausalConvTranspose1d(nnx.Module):
    ...
    def __init__(
        ...
        self.conv = ConvTranspose1d(
            in_channels, out_channels, kernel_size, stride, dtype=dtype, rngs=rngs
        )
        self.norm = nnx.GroupNorm(
            num_features=out_channels, num_groups=1, epsilon=1e-5, param_dtype=dtype, rngs=rngs
        )
        self.kernel_size = kernel_size
        self.stride = stride
...
class TransformerVocos(nnx.Module):
    ...
    def __init__(
        ...
        self.layer_norm = nnx.LayerNorm(
            config.vocoder_dim, epsilon=1e-6, param_dtype=dtype, rngs=rngs
        )
...
class AudioEncoder(nnx.Module):
    ...
    def __init__(
        ...
        if config.avg_pooler != 1:
            ...
            self.down_norm = nnx.LayerNorm(
                config.d_model, epsilon=1e-6, param_dtype=dtype, rngs=rngs
            )
        else:
    ...
    def _load_codebook_weights(self, model_config):
        ...
        with safe_open(safetensors_path, framework="numpy") as f:
            for i in range(self.config.num_quantizers):
                ...
                if hf_key in f.keys():  # noqa: SIM118
