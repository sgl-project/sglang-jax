...
class ModelConfigRegistry:
    """Registry that maps model names to their model configs.
    ...
    def register(cls, model_name: str, config_factory: callable) -> None:
        ...
        logger.info("Registered model config '%s'", model_name)

    @classmethod
    def get_config(cls, model_path: str) -> WanModelConfig:
        """Get the model config for a given model path.
        ...
        raise ValueError(
            f"No model config found for model '{model_path}'. "
            f"Available models: {available_models}. "
            f"You can register new models using ModelConfigRegistry.register()."
        )
...
def get_model_config(model_path: str) -> WanModelConfig:
    ...
    Returns:
        A model config instance configured for the specified model.
    """
    return ModelConfigRegistry.get_config(model_path)
