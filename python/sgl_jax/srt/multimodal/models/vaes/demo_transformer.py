...
def get_transformer_output(path):
    ...
    # (1, 2, 3, 4, 16)
    latents_original = (
        torch.tensor(np.arange(1 * 1 * 3 * 4 * 16), dtype=torch.float32)
        .reshape(1, 1, 3, 4, 16)
        .permute((0, 4, 1, 2, 3))
    )
    y = vae.decode(latents_original)
...
def get_jax_output(path):
    ...
    x = jnp.array(np.arange(1 * 1 * 3 * 4 * 16), dtype=jnp.float32).reshape(1, 1, 3, 4, 16)
