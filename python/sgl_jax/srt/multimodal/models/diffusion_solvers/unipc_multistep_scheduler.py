...
class UniPCMultistepScheduler(SchedulerMixin):
    """
    ...
    def __init__(
        ...
        self.dtype = params.pop("dtype")  # dtype not in config
    ...
    def create_state(
        self, common: CommonSchedulerState | None = None
    ...
    def set_timesteps(
        ...
        if self.config.use_flow_sigmas:
            ...
            if self.config.final_sigmas_type == "sigma_min":
                sigma_last = (
                    (1 - state.common.alphas_cumprod[0]) / state.common.alphas_cumprod[0]
                ) ** 0.5
            elif self.config.final_sigmas_type == "zero":
                sigma_last = 0
            else:
        ...
        timestep_list = jnp.zeros(
            (self.config.solver_order,), dtype=jnp.int32
        )  # Timesteps are integers
    ...
    def multistep_uni_c_bh_update(
        ...
        x = last_sample if last_sample is not None else jnp.zeros_like(this_sample)
