...
# ----------------------------------------------------------------------------------------------
# Qwen2.5-VL vision config (staged understanding path). Restored on epic/support_mimo_v2.5_vlm:
# the in-model Qwen2.5-VL was reverted to the staged (vit -> auto_regressive) pipeline, whose
# vit_model_runner builds its QwenVLModelVitConfig via get_qwen_vl_config(model_path).
# ----------------------------------------------------------------------------------------------
...
def qwen_vl_vision_config_from_hf(hf_config) -> QwenVLModelVitConfig:
    ...
    local dir). Without this override the bare ``QwenVLModelVitConfig()`` default
    ``hidden_size=3584`` is wrong for the 7B ViT (real vision width is 1280; 3584 is the
    post-merger LLM dim), so the ViT patch_embed reshape fails. ``out_hidden_size`` falls
    back to the top-level ``hidden_size``.
    """
...
class QwenVLConfigRegistry:
    ...
    def get_config(cls, model_path: str) -> QwenVLModelVitConfig:
        ...
        available_models = list(cls._REGISTRY.keys())
        raise ValueError(
            f"No QwenVL config found for model '{model_path}'. "
            f"Available models: {available_models}. "
            f"You can register new models using QwenVLConfigRegistry.register()."
        )
...
def get_qwen_vl_config(model_path: str) -> QwenVLModelVitConfig:
    """Convenience function to get the Qwen2.5-VL vision config for the staged vit stage."""
