...
class AudioBackboneScheduler:
    ...
    def _create_forward_batch(
        ...
        if is_prefill:
            padded_input_groups = self._find_padding_size(
                actual_T_groups, self.input_group_paddings
            )
        ...
        if is_prefill and actual_T_groups < T_groups:
            ...
            logger.debug("Created attention mask: %d/%d valid groups", actual_T_groups, T_groups)
    ...
    def _prepare_batch(self, req: Req) -> tuple[jax.Array, ForwardBatch, LogitsMetadata]:
        ...
        if is_prefill:
            ...
            if padded_groups > T_groups:
                ...
                logger.debug("Padded from %d to %d groups", actual_T_groups, padded_groups)

        forward_batch = self._create_forward_batch(
            input_ids, rid, T_groups, actual_T_groups, is_prefill
        )
        ...
        for req in batch:
            ...
            while True:
                ...
                if text_token_id in self.eos_token_ids:
                    logger.info(
                        "Generation finished for rid=%s at step %d (EOS)", req.rid, step_count
                    )
