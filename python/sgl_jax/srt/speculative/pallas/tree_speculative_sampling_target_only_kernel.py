...
def _tree_speculative_sampling_target_only_kernel(
    ...
    *,
    ...
    def probs_cumsum(arr):
        ...
        return csum_arr
        # def cumulative_sum_step(current_sum, element):
        #     new_sum = current_sum + element
        #     return new_sum, new_sum
        # final_sum, cumulative_sums = scan(
        #     f=cumulative_sum_step,
        #     init=jnp.array(0),
        #     xs=arr
        # )
    ...
    def _sampling_from_prob(probs, threshold):
        print(f"{probs.shape=} {threshold.shape=}")
        valid_probs = jnp.where(probs > 0, probs, 0)
        print(f"{valid_probs.shape=}")
