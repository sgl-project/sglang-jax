...
class DraftForwardPlan:
    ...
    forward_metadata: PagedKVLayout
    seq_lens: np.ndarray
...
class DFlashWorker(BaseSpecWorker, BaseDraftWorker):
    ...
    def __init__(self, server_args, target_worker: ModelWorker):
        ...
        if (
            not self.speculative_algorithm.is_dflash()
            or self.topk != 1
            or self.speculative_num_steps != 1
        ):
            raise ValueError("DFlashWorker only supports DFLASH with topk=1 and num_steps=1.")
    ...
    def _build_draft_forward_plan(
        ...
        metadata = self.draft_model_runner.attn_backend.prepare_paged_kv_layout(
            draft_mwb,
            page_indices=page_indices,
            fixed_capacity=self._page_indices_capacity(bs),
        )
    ...
    def _build_target_verify_plan(
        ...
        return TargetVerifyPlan(
            ...
            active_mask=draft_plan.relay_valid_mask,
    ...
    def _init_jit_draft_block(self):
        ...
        def draft(
            ...
            *,
            ...
            forward_batch.attn_backend.forward_metadata = build_target_verify_metadata(
                forward_batch.attn_backend.forward_metadata,
                target_prefix_lens,
                allocated_lens,
                active_mask=relay_valid_mask,
            ...
            cache_offsets = jnp.clip(
                cache_offsets,
                0,
                cache_rows.shape[1] - block_size,
            )
            cache_indices = (
                cache_offsets[:, None] + jnp.arange(block_size, dtype=jnp.int32)[None, :]
            )
            row_indices = jnp.arange(cache_rows.shape[0], dtype=jnp.int32)[:, None]
            selected_cache_loc = cache_rows.at[row_indices, cache_indices].get(
                out_sharding=cache_row_sharding,
            )
            selected_cache_loc = jnp.where(
    ...
    def _precompile_dflash_variant(self, bs: int) -> None:
        ...
        forward_batch.forward_mode = ForwardMode.TARGET_VERIFY
        draft_metadata = self.draft_model_runner.attn_backend.prepare_paged_kv_layout(
            draft_batch,
            page_indices=page_indices,
            fixed_capacity=self._page_indices_capacity(bs),
        )
        ...
        active_mask = jax.device_put(prefix_lens > 0, data_sharding)
        if use_relay_state:
