...
class BatchedMinNewTokensPenalizer(_BatchedPenalizer):
    ...
    def _apply(self, logits: np.ndarray) -> np.ndarray:
        # Convert numpy arrays to JAX for GPU operations
        len_output_tokens_jax = np.array(self.len_output_tokens)
        min_new_tokens_jax = np.array(self.min_new_tokens)
        stop_token_penalties_jax = np.array(self.stop_token_penalties)

        # Create mask for requests that haven't reached min_new_tokens
        mask = len_output_tokens_jax < min_new_tokens_jax

        # Expand mask to match logits shape
        mask_expanded = np.broadcast_to(mask, logits.shape)

        # Apply penalties where mask is True
        penalty_to_add = np.where(mask_expanded, stop_token_penalties_jax, 0.0)

        return logits + penalty_to_add
    ...
    def _filter(self, keep_indices: np.ndarray):
        ...
        self.min_new_tokens = self.min_new_tokens[keep_indices_np]
        self.stop_token_penalties = self.stop_token_penalties[keep_indices_np]
        self.len_output_tokens = self.len_output_tokens[keep_indices_np]
