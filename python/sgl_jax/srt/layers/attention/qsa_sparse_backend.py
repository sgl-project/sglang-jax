...
class QSASparseAttentionBackend(FlashAttention):
    ...
    def __call__(
        ...
        **qsa_kwargs,
        ...
        ``q_idx`` (scoring queries), and ``compressed`` / ``groups`` /
        ``seq_ids`` / ``ring`` from ``QSAIndexer.compress_batch``.
        """
        slot = self.full_slot.get(getattr(layer, "layer_id", -1))
        q_idx = qsa_kwargs.get("q_idx")
        if slot is None or q_idx is None:
        ...
        out = self._run_sparse(q, q_idx, compressed_cache, kv_fused, layer, forward_batch)
    ...
    def _run_sparse(self, q, q_idx, compressed_cache, kv_fused, layer, forward_batch):
        ...
        def _select_and_attend(
            q_, q_idx_, comp_, kv_, positions_, t2r_, pt_, sl_, pi_, cuq_, dist_
        ):
            block_ids = select_blocks(
                ...
                pi_,
                cuq_,
                cuq_,  # unused on the kernel path, which walks the fixed-stride table
                dist_,
                block_topk=self.block_topk,
                compress_ratio=self.compress_ratio,
