...
from sgl_jax.srt.layers.attention.flash_attn_kernel.v2.tuned_block_sizes import (
...
def ref_ragged_paged_attention(
    ...
    *,
    ...
    assert cache_num_kv_heads == actual_num_kv_heads
    ...
    for i in range(distribution[-1]):
        ...
        # Update the kv cache separately for K and V.
        assert kv_len - q_len >= 0

        # Update K cache
        gathered_k = k_cache[indices]
        gathered_k_shape = gathered_k.shape
        gathered_k = gathered_k.reshape(-1, *gathered_k_shape[-2:])
        gathered_k = gathered_k.at[kv_len - q_len : kv_len].set(
            keys[q_start:q_end, :, :actual_head_dim]
        )
        k_cache = k_cache.at[indices].set(gathered_k.reshape(gathered_k_shape))

        # Update V cache
        gathered_v = v_cache[indices]
        gathered_v_shape = gathered_v.shape
        gathered_v = gathered_v.reshape(-1, *gathered_v_shape[-2:])
        gathered_v = gathered_v.at[kv_len - q_len : kv_len].set(
            values[q_start:q_end, :, :actual_head_dim]
        )
        v_cache = v_cache.at[indices].set(gathered_v.reshape(gathered_v_shape))

        # Get K and V from the updated cache
        k = gathered_k[:kv_len, :, :actual_head_dim]
        v = gathered_v[:kv_len, :, :actual_head_dim]
...
def _ragged_paged_attention_kernel(
    ...
    k_hbm_ref,  # [max_num_tokens, actual_num_kv_heads, head_dim] - separate K input
    v_hbm_ref,  # [max_num_tokens, actual_num_kv_heads, head_dim] - separate V input
    k_cache_hbm_ref,  # [total_num_pages, page_size, actual_num_kv_heads, head_dim] - separate K cache
    v_cache_hbm_ref,  # [total_num_pages, page_size, actual_num_kv_heads, head_dim] - separate V cache
    # Output
    o_hbm_ref,  # [actual_num_kv_heads, max_num_tokens, num_q_heads_per_kv_head // q_packing, q_packing, head_dim]
    updated_k_cache_hbm_ref,  # [total_num_pages, page_size, actual_num_kv_heads, head_dim] - separate updated K cache
    updated_v_cache_hbm_ref,  # [total_num_pages, page_size, actual_num_kv_heads, head_dim] - separate updated V cache
    # Scratch
    # bkv_x2_ref,  # [2, bkv_sz, actual_num_kv_heads, head_dim] - now for separate K/V processing
    bk_x2_ref,  # [2, bkv_sz, actual_num_kv_heads, head_dim] - now for separate K/V processing
    bv_x2_ref,  # [2, bkv_sz, actual_num_kv_heads, head_dim] - now for separate K/V processing
    ...
    *,
    ...
    assert k_cache_hbm_ref.shape == v_cache_hbm_ref.shape
    ...
    # Get shape info from separate K cache (V cache has same shape)
    (
        total_num_pages,
        page_size,
        cache_num_kv_heads,
    ...
    k_dtype = k_hbm_ref.dtype
    v_dtype = v_hbm_ref.dtype
    kv_dtype = k_dtype  # For compatibility with existing code that expects kv_dtype
    assert o_hbm_ref.dtype == q_dtype
    assert k_dtype == v_dtype  # K and V should have same dtype
    assert get_dtype_packing(q_dtype) == q_packing

    # Define packing variables
    kv_packing = get_dtype_packing(kv_dtype)
    ...
    def _fetch_bk(seq_idx, bkv_idx, bkv_sem_idx, *, wait=False):
        sem = sems.at[0, bkv_sem_idx]
        vmem_ref = bk_x2_ref.at[bkv_sem_idx]  # This will hold K data
        ...
        debug_print(
            "[RPA debug]"
            f" -----------{'wait' if wait else 'start'}_fetch_bk-----------"
        )
        ...
        # Fetch K data directly from new K.
        @pl.when(bkv_sz_frm_new > 0)
        def _fetch_bk_from_new_k():
    ...
    def _fetch_bv(seq_idx, bkv_idx, bkv_sem_idx, *, wait=False):
        sem = sems.at[0, bkv_sem_idx]
        vmem_ref = bv_x2_ref.at[
            bkv_sem_idx + 1
        ]  # This will hold V data (use next buffer)
        ...
        debug_print(
            "[RPA debug]"
            f" -----------{'wait' if wait else 'start'}_fetch_bv-----------"
        )
        debug_print("[RPA debug] seq_idx={}", seq_idx)
        debug_print("[RPA debug] bkv_idx={}", bkv_idx)
        debug_print("[RPA debug] bkv_sem_idx={}", bkv_sem_idx)
        ...
        # Fetch V data directly from new V.
        @pl.when(bkv_sz_frm_new > 0)
        def _fetch_bv_from_new_v():
            new_kv_len_start = q_end - kv_left_frm_new
            debug_print("[RPA debug] new_kv_len_start={}", new_kv_len_start)
            debug_print("[RPA debug] offset_in_bkv={}", offset)
            _async_copy(
    ...
    def _update_k_cache(seq_idx, bkv_sem_idx, offset, update_sz, *, wait=False):
        sem = sems.at[3, bkv_sem_idx]
        vmem_ref = bk_x2_ref.at[bkv_sem_idx]  # K data buffer
        ...
        debug_print(
            "[RPA debug]"
            f" -----------{'wait' if wait else 'start'}_update_k_cache-----------"
        )
        debug_print("[RPA debug] seq_idx={}", seq_idx)
        debug_print("[RPA debug] bkv_sem_idx={}", bkv_sem_idx)
        ...
        def loop_body(i, states):
            ...
            _async_copy(
                ...
                updated_k_cache_hbm_ref.at[
                    page_indices_ref[page_indices_offset + i], pl.ds(ignore, sz)
                ],
                sem,
                wait,
            )
    ...
    def _update_v_cache(seq_idx, bkv_sem_idx, offset, update_sz, *, wait=False):
        sem = sems.at[3, bkv_sem_idx]
        vmem_ref = bv_x2_ref.at[bkv_sem_idx + 1]  # V data buffer (use next buffer)
        bkv_id = offset // bkv_sz
        kv_p_start = offset // page_size
        kv_p_end = cdiv(offset + update_sz, page_size)
        ignore = offset % page_size
        ...
        debug_print(
            "[RPA debug]"
            f" -----------{'wait' if wait else 'start'}_update_v_cache-----------"
        )
        debug_print("[RPA debug] seq_idx={}", seq_idx)
        debug_print("[RPA debug] bkv_sem_idx={}", bkv_sem_idx)
        debug_print("[RPA debug] offset={}", offset)
    ...
    def start_fetch_bkv(seq_idx, bkv_idx, bkv_sem_idx):
        k_result = _fetch_bk(seq_idx, bkv_idx, bkv_sem_idx)
        v_result = _fetch_bv(seq_idx, bkv_idx, bkv_sem_idx)
        return k_result  # Both should return the same offset/size info

    def wait_fetch_bkv(seq_idx, bkv_idx, bkv_sem_idx):
        k_result = _fetch_bk(seq_idx, bkv_idx, bkv_sem_idx, wait=True)
        v_result = _fetch_bv(seq_idx, bkv_idx, bkv_sem_idx, wait=True)
        return k_result  # Both should return the same offset/size info
    ...
    def start_update_kv_cache(seq_idx, bkv_sem_idx, offset, update_sz):
        ...
        _update_k_cache(seq_idx, bkv_sem_idx, offset, update_sz)
        _update_v_cache(seq_idx, bkv_sem_idx, offset, update_sz)
    ...
    def wait_update_kv_cache(bkv_sem_idx):
        ...
        def _():
            ...
            _update_k_cache(seq_idx, bkv_sem_idx, offset, update_sz, wait=True)
            _update_v_cache(seq_idx, bkv_sem_idx, offset, update_sz, wait=True)
    ...
    def strided_load_bkv(bkv_sem_idx, start, step, *, bkv_bitmask):
        ...
        if kv_packing == 1:
            ...
            # k2 = strided_load(k_ref, start + 1, step, dtype=kv_dtype)
            return [_mask_kv(k, v)]
...
def prepare_inputs(
    ...
    # Process query tensor with padding and reshaping for TPU optimization
    q = (
        jnp.pad(
            q.reshape(
                max_num_tokens,
    ...
    # Process K and V separately (no merge!)
    # TODO(kyuyeunk, chengjiyao): Add kv quantization here.
    k_processed = jnp.pad(
        k, ((0, 0), (0, 0), (0, head_dim - actual_head_dim)), constant_values=0
    )
    v_processed = jnp.pad(
        v, ((0, 0), (0, 0), (0, head_dim - actual_head_dim)), constant_values=0
    )

    return q, k_processed, v_processed
...
def ragged_paged_attention(
    ...
    *,
    ...
    actual_num_q_heads_per_kv_head = actual_num_q_heads // actual_num_kv_heads
    q, k_processed, v_processed = prepare_inputs(q, k, v)
    (
        _,
        max_num_tokens,
        num_q_heads_per_kv_head_per_q_packing,
    ...
    if vmem_limit_bytes is None:
        vmem_limit_bytes = int(
            ...
            * 2.4
        )  # TODO(jevinjiang): figure out why it is so inaccurate?
    grid = (distribution[2],)

    in_specs = [
        pl.BlockSpec(memory_space=pltpu.HBM),  # q
        pl.BlockSpec(memory_space=pltpu.HBM),  # k_processed
        pl.BlockSpec(memory_space=pltpu.HBM),  # v_processed
        pl.BlockSpec(memory_space=pltpu.HBM),  # k_cache
        pl.BlockSpec(memory_space=pltpu.HBM),  # v_cache
    ]

    out_specs = [
        pl.BlockSpec(memory_space=pltpu.HBM),  # output
        pl.BlockSpec(memory_space=pltpu.HBM),  # updated k_cache
        pl.BlockSpec(memory_space=pltpu.HBM),  # updated v_cache
    ]

    bk_double_buf = pltpu.VMEM(
        (2, bkv_sz, *k_cache.shape[2:]),  # [2, bkv_sz, actual_num_kv_heads, head_dim]
        k_cache.dtype,
    )

    bv_double_buf = pltpu.VMEM(
        (2, bkv_sz, *v_cache.shape[2:]),  # [2, bkv_sz, actual_num_kv_heads, head_dim]
        v_cache.dtype,
    )
    ...
    scratch_shapes = [
        bk_double_buf,  # Double buffering for k block.
        bv_double_buf,  # Double buffering for v block.
    ...
    kernel = jax.named_scope(scope_name)(
        pl.pallas_call(
            ...
            input_output_aliases={
                ...
                11: 1,
                12: 2,
            },  # k_cache->updated_k_cache, v_cache->updated_v_cache
            name=scope_name,
        )
    )

    output, updated_k_cache, updated_v_cache = kernel(
        *scalar_prefetches, q, k_processed, v_processed, k_cache, v_cache
    )
    return (
