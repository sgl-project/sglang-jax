...
def static_validate_inputs(
    ...
    *,
    ...
    # Fused KV cache has num_kv_heads * 2 heads (K heads + V heads)
...
def ref_ragged_paged_attention(
    ...
    *,
    ...
    num_kv_heads = fused_kv_heads // 2  # Split fused KV heads back to separate counts
    ...
    for i in range(num_seqs[0]):
        ...
        # Extract from interleaved format: k1,v1,k2,v2,k3,v3 -> separate K and V
        # Use reshape to avoid strided access issues
        seq_len, fused_heads, head_dim = fused_kv.shape
        num_kv_heads = fused_heads // 2

        # Reshape to separate K and V: [seq_len, num_heads, 2, head_dim]
        kv_reshaped = fused_kv.reshape(seq_len, num_kv_heads, 2, head_dim)

        # Extract K and V data
        k = kv_reshaped[:, :, 0, :]  # K heads
        v = kv_reshaped[:, :, 1, :]  # V heads
...
def ragged_paged_attention_kernel(
    ...
    # Scratch
    kv_bufs,  # [2, num_kv_pages_per_blk, page_size, num_kv_heads_per_blk * 2, head_dim] - Interleaved K&V
    sems,  # [2, 1] - Single semaphore for interleaved KV operation
    ...
    *,
    ...
    # Fused KV buffer: [2, num_kv_pages_per_blk, page_size, num_kv_heads_per_blk * 2, head_dim]
    _, num_kv_pages_per_blk, page_size, fused_kv_heads_per_blk, _ = kv_bufs.shape
    num_kv_heads_per_blk = fused_kv_heads_per_blk // 2  # Split back to K and V
    ...
    def create_fused_kv_async_copy_descriptor(
        ...
        # For fused KV cache: copy both K and V heads together in interleaved format
        # KV layout: [k1, v1, k2, v2, k3, v3, ...] interleaved K and V heads
        fused_heads_start = (
            heads_blk_idx * num_kv_heads_per_blk * 2
        )  # *2 because K+V are interleaved
        fused_heads_count = num_kv_heads_per_blk * 2  # Both K and V heads interleaved

        async_copy_fused_kv = MultiPageAsyncCopyDescriptor(
            kv_cache_hbm_ref.at[:, :, pl.ds(fused_heads_start, fused_heads_count), :],
            kv_bufs.at[buf_idx],
            sems.at[buf_idx, 0],  # Single semaphore for fused operation
            page_indices_ref,
            metadata,
        )
    ...
    def batch_load_all_heads_kv(k_ref, v_ref, num_kv_heads_per_blk):
        ...
        for head_idx in range(num_kv_heads_per_blk):
            # k_ref and v_ref are already separated from the interleaved data
            # so we can use the original indexing logic
    ...
    *,
    ...
    Args:
      ...
      kv_cache: paged KV cache with K and V interleaved on head dimension.
                Layout: [k1, v1, k2, v2, k3, v3, ...]. Uses bitcast for efficient extraction.
    ...
    # Fused KV buffer: contains both K and V data in single buffer
    fused_kv_buf_scratch = pltpu.VMEM(
        (
            2,  # For double buffering during DMA copies.
            num_kv_pages_per_blk,
            page_size,
            num_kv_heads_per_blk * 2,  # * 2 for both K and V heads
            head_dim,
        ),
        kv_cache.dtype,
    )
