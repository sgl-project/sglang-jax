...
def select_blocks(
    ...
    *,
    ...
    ``sum_h relu(q_h . k) * w_h`` with the relu *before* the head sum, and its
    ``compression_ratio`` makes entry ``e`` visible only when
    ``e < (q_pos + 1) // compression_ratio``. QSA's score is
    ``sum_h relu(q_h . k)`` under that same visibility rule, so all-ones
