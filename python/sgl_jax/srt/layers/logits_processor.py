...
class LogitsProcessor(nnx.Module):
    ...
    def __call__(
        ...
        if not logits_metadata.extend_return_logprob:
            ...
            indices = (np.arange(input_logprobs.shape[0]), logits_metadata.extend_input_logprob_token_ids_device)
    ...
    @staticmethod
    def get_token_ids_logprobs(all_logprobs: jax.Array, logits_metadata: LogitsMetadata, mesh: Mesh):
        ...
        for token_ids, pruned_len in zip(
            ...
            input_token_ids_logprobs_val.append(
                [all_logprobs.at[pt + j, token_ids].get(out_sharding = out_sharding) for j in range(pruned_len)]
            )
