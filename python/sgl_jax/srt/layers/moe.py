...
class GateLogit(nnx.Module):
    ...
    def __init__(
        ...
        self.kernel = nnx.Param(
            jit_with_partitioning(nnx.initializers.normal(), self.kernel_axes)(
        ...
        if self.use_bias:
            ...
            self.bias = nnx.Param(
                jit_with_partitioning(nnx.initializers.zeros_init(), bias_axes)(
...
class EPMoE(nnx.Module):
    def __init__(
        ...
        *,
        ...
        self.wi_0 = nnx.Param(
            jit_with_partitioning(nnx.initializers.normal(), expert_kernel_axes)(
                rngs.params(),
    ...
    def _single_expert_forward(self, inputs, router_logits):
    ...
    def _permute(self, inputs, top_k_indices, top_k_weights):
        ...
        sorted_inputs = jnp.take(inputs_2d, indices=sorted_indices, axis=0)
...
class FusedMoE(nnx.Module):
    def __init__(
        ...
        *,
        ...
        self.wi_sharding = (None, None, "tensor")
        self.wo_sharding = (None, "tensor", None)

        self.wi_0 = nnx.Param(
            jit_with_partitioning(nnx.initializers.normal(), self.wi_sharding)(
                rngs.params(),
                (num_experts, config.hidden_size, intermediate_dim),
                weight_dtype,
            )
        ...
        self.wo = nnx.Param(
            jit_with_partitioning(nnx.initializers.normal(), self.wo_sharding)(
                rngs.params(),
                (num_experts, intermediate_dim, config.hidden_size),
                weight_dtype,
            )
