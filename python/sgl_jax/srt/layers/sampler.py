...
class Sampler(nnx.Module):
    ...
    def _apply_individual_penalties(self, operands):
        ...
        def _apply_freq_penalty(ops):
            _logits, _sm = ops
            if _sm.cumulated_frequency_penalties is None:
                return _logits
            return _logits - _sm.cumulated_frequency_penalties.astype(_logits.dtype)
        ...
        def _apply_pres_penalty(ops):
            _logits, _sm = ops
            if _sm.cumulated_presence_penalties is None:
                return _logits
            return _logits - _sm.cumulated_presence_penalties.astype(_logits.dtype)
