...
class LlamaEagleModel(LlamaModel):
    def __init__(
        ...
        super().__init__(config=config, dtype=dtype, rngs=rngs, is_draft_model=True)
        self.config = config
        ...
        self.midlayer = LlamaDecoderLayer(config=config, layer_id=0, rngs=rngs, dtype=dtype)
...
class LlamaForCausalLMEagle3(LlamaForCausalLM):
    def __init__(
        ...
        self.dtype = dtype
        self.transformer = LlamaEagleModel(config, dtype=dtype, rngs=rngs)
        # Llama 3.2 1B Instruct set tie_word_embeddings to True
        # Llama 3.1 8B Instruct set tie_word_embeddings to False
        self.load_lm_head_from_target = False
        if self.config.tie_word_embeddings:
            self.lm_head = self.transformer.embed_tokens
        else:
    ...
    def _create_llama_ealge3_weight_mappings(self):
        ...
        # mappings["t2d"] = WeightMapping(
        #     target_path="hot_token_ids",
        #     sharding=(None,),
        #     transpose=False,
        # )
        mappings["fc.weight"] = WeightMapping(
            target_path="transformer.fc.weight",
            sharding=(None, None),
            transpose=True,
        )
        ...
        mappings["norm.weight"] = WeightMapping(
            target_path="transformer.norm.scale",
            sharding=(None,),
            transpose=False,
        )
        mappings["midlayer.hidden_norm.weight"] = WeightMapping(
            target_path="transformer.midlayer.hidden_norm.scale",
            sharding=(None,),
            transpose=False,
        )
        mappings["midlayer.input_layernorm.weight"] = WeightMapping(
            target_path="transformer.midlayer.input_layernorm.scale",
            sharding=(None,),
            transpose=False,
        )
        mappings["midlayer.mlp.down_proj.weight"] = WeightMapping(
            target_path="transformer.midlayer.mlp.down_proj.weight",
            sharding=(None, None),
            transpose=True,
        )
        mappings["midlayer.mlp.gate_proj.weight"] = WeightMapping(
            target_path="transformer.midlayer.mlp.gate_proj.weight",
            sharding=(None, None),
            transpose=True,
        )
        mappings["midlayer.mlp.up_proj.weight"] = WeightMapping(
            target_path="transformer.midlayer.mlp.up_proj.weight",
            sharding=(None, None),
            transpose=True,
        )
        mappings["midlayer.post_attention_layernorm.weight"] = WeightMapping(
            target_path="transformer.midlayer.post_attention_layernorm.scale",
            sharding=(None,),
            transpose=False,
        )
