"""Qwen2.5-VL concrete vision encoder plugin.
...
class Qwen25VLVisionEncoderPlugin(VisionEncoderPlugin):
    ...
    (``output_modality`` / ``feature_dim`` / ``get_num_output_tokens``). The
    scheduler owns pixels/valid/merge indices and the encode-input pytree; this
    ...
    def __init__(self, model_config):
        ...
        if vision_cfg is None:
            raise ValueError(
                "Qwen2.5-VL vision encoder plugin requires " "model_config.hf_config.vision_config."
            )
        ...
        self.in_channels = int(getattr(vision_cfg, "in_channels", 3))
        ...
        # Trailing dim of a single patch feature row, i.e. the second axis of a
        # real ``item.feature``: in_channels * temporal_patch_size * patch_size**2.
        self.feature_dim = self.in_channels * self.temporal_patch_size * self.patch_size**2
        # Last axis of ``rotary_pos_emb`` (2D rope: hpos/wpos halves concatenated).
    ...
    def _get_image_metadata(self, item) -> Qwen25VLVisionMetadata:
        ...
        if feature is None or np.asarray(feature).shape[0] != expected_patch_rows:
            actual = None if feature is None else np.asarray(feature).shape[0]
            raise ValueError(
