...
The text/MoE backbone is reused verbatim from :mod:`mimo_v2_pro`; this module
adds the MiMoV2 vision tower and wires it onto the in-model multimodal contract
(:class:`InModelMultimodalContract`).

The vision compute (GQA attention with optional first-key sink biases, per-layer
...
def _encode_first_key_attention_bias(
    ...
    q = jnp.concatenate((q, jnp.ones((*q.shape[:-1], 1), dtype=q.dtype)), axis=-1)
    k = jnp.concatenate((k, key_bias[..., None]), axis=-1)
    v = jnp.concatenate((v, jnp.zeros((*v.shape[:-1], 1), dtype=v.dtype)), axis=-1)
    return q, k, v
