...
class QWen3Attention(nnx.Module):
    ...
    def __call__(
        ...
        attn_output, fused_kv_buffer = self.attn(q, k, v, forward_batch=forward_batch)

        output, _ = self.o_proj(attn_output)
        return output, fused_kv_buffer
...
class QWen3DecoderLayer(nnx.Module):
    ...
    def __call__(
        ...
        hidden_states, fused_kv_buffer = self.self_attn(
            positions=positions,
        ...
        return hidden_states, residual, fused_kv_buffer, layer_callback_flag
...
class QWen3Model(nnx.Module):
    ...
    def __call__(
        ...
        for layer in self.layers:
            hidden_states, residual, fused_kv_buffer, callback_flag = layer(
                forward_batch.positions, hidden_states, forward_batch, residual
            )
            layers_kv.append(fused_kv_buffer)
