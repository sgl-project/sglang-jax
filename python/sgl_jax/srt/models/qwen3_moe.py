...
class Qwen3MoeForCausalLM(nnx.Module):
    ...
    def __call__(
        ...
        if os.environ.get("SGL_QWEN3_MOE_DEBUG_MODEL_STAGE", "0") == "1":
            jax.debug.print(
                "QWEN3_MOE_MODEL_STAGE after_transformer hidden_shape={}",
                hidden_states.shape,
            )
        if not getattr(self.config, "tie_word_embeddings", False):
        ...
        if os.environ.get("SGL_QWEN3_MOE_DEBUG_MODEL_STAGE", "0") == "1":
            jax.debug.print(
                "QWEN3_MOE_MODEL_STAGE after_logits next_logits_shape={}",
