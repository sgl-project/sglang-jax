    ...
    def _create_layer_mappings(self, layer_idx: int) -> dict:
        ...
        if use_fused:
            ...
            mappings[f"__MOE_EXPERTS__{prefix}.block_sparse_moe.experts.w2"] = WeightMapping(
                ...
                transpose=True,
                concat_axis=-1,  # Grok w2 needs TP concat?
                # Grok w2 is (output=hidden, input=inter). Transpose -> (inter, hidden).
                # We want to split 'inter' (dim 1).
                # If original is (H, I), TP splits I (dim 1).
                # Transpose makes it (I, H). TP splits I (dim 0).
                # Wait, `concat_axis` logic in loader:
                # if concat_axis is set, it concats shards.
                # Grok w2 shards are partial along 'inter'.
                # If we transpose first, we have partial (I, H). We concat along 0.
                # But `WeightLoader` does:
                # if transpose: transpose(weight).
                # concat is done BEFORE transpose if concat_axis is passed?
                # In `load_weights_from_safetensors`:
                # if concat_axis: concat(weights, axis=concat_axis) -> THEN transpose.
                # So if shards are (H, I/tp), we concat along 1 -> (H, I).
                # THEN transpose -> (I, H).
                # So concat_axis should be 1 (or -1).
            )
