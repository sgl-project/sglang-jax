...
class QWen3OmniMoeAttention(nnx.Module):
    def __init__(
        self,
        hidden_size: int,
        num_heads: int,
        ...
        self.rotary_emb = MRotaryEmbedding(
            ...
            mrope_interleaved=rope_scaling.get("mrope_interleaved") if rope_scaling else False,
        )
...
class Qwen3OmniMoeThinkerTextMLP(nnx.Module):
    def __init__(
        self,
        hidden_size: int,
...
class Qwen3OmniMoeThinkerTextDecoderLayer(nnx.Module):
    def __init__(
...
class Qwen3OmniMoeThinkerTextModel(nnx.Module):
    def __init__(
        ...
        self.norm = RMSNorm(
            config.hidden_size, epsilon=config.rms_norm_eps, param_dtype=dtype, dtype=dtype
        )
...
class Qwen3OmniMoeForConditionalGeneration(nnx.Module):
    def __init__(
        ...
        self.config = (
            config
            if not getattr(config, "thinker_config", None)
            else config.thinker_config.text_config
        )
        self.dtype = dtype
        logger.info("QWen3OmniMoeThinkerForCausalLM config dtype: %s", self.dtype)
        self.model = Qwen3OmniMoeThinkerTextModel(self.config, dtype=self.dtype, mesh=mesh)
    ...
    def load_weights(self, model_config: ModelConfig):
        ...
        weight_mappings = self._create_qwen3_omni_moe_weight_mappings()
        loader.load_weights_from_safetensors(weight_mappings)
        logger.info("Qwen3OmniMoeThinker weights loaded successfully!")

    def _create_qwen3_omni_moe_weight_mappings(self) -> dict:
        mappings = {
