...
class MiMoV2Moe(nnx.Module):
    ...
    def __init__(
        ...
        if self.use_fused:
            ...
            # Kernel requires subc_quant_wsz to be a multiple of 256 for t_packing=2.
            ...
            if moe_quant_block_size == 128 and not is_static_quant_ckpt:
                self.experts.subc_quant_wsz = 256
        else:
            self.experts = EPMoE(
                hidden_size=config.hidden_size,
                num_experts=num_experts,
    ...
    def __call__(
        ...
        if self.layer_id == 1:
            jax.debug.print(
                "MiMoV2Moe layer=1 stats hidden_absmax={h} router_absmax={r}",
                h=jnp.max(jnp.abs(hidden_states.astype(jnp.float32))),
                r=jnp.max(jnp.abs(router_logits.astype(jnp.float32))),
            )
        ...
        o_nan = jnp.isnan(mlp_output).sum()
        o_inf = jnp.isinf(mlp_output).sum()
        jax.lax.cond(
            (o_nan > 0) | (o_inf > 0),
            lambda _: jax.debug.print(
                "MiMoV2Moe layer={lid} mlp_out_nan={o_nan} mlp_out_inf={o_inf}",
                lid=self.layer_id,
                o_nan=o_nan,
                o_inf=o_inf,
            ),
...
class MiMoMoeAttention(nnx.Module):
    ...
    def __call__(
        ...
        # Debug: check input hidden_states and v_proj weights for layer 0 and 1
        _dbg_attn = self.layer_id in (0, 1)
        if _dbg_attn:
            jax.debug.print(
                "[ATTN_PRE] layer={lid} hidden_states_absmax={hsa}",
                lid=self.layer_id,
                hsa=jnp.abs(hidden_states).max(),
            )
            # Check v_proj weight statistics
            if hasattr(self.v_proj, "weight_q"):
                # FP8 quantized path
                v_wq = self.v_proj.weight_q.value
                v_ws = self.v_proj.weight_scale.value
                # Cast FP8 to float32 to get actual magnitude
                v_wq_f32 = v_wq.astype(jnp.float32)
                jax.debug.print(
                    "[ATTN_PRE] layer={lid} v_proj FP8: weight_q_shape={wqs} weight_scale_shape={wss}"
                    " weight_q_absmax={wqa} weight_scale_mean={wsm} weight_scale_absmax={wsa}"
                    " weight_block_size={wbs}",
                    lid=self.layer_id,
                    wqs=v_wq.shape,
                    wss=v_ws.shape,
                    wqa=jnp.abs(v_wq_f32).max(),
                    wsm=v_ws.mean(),
                    wsa=jnp.abs(v_ws).max(),
                    wbs=str(getattr(self.v_proj, "weight_block_size", None)),
                )
                # Also check k_proj for comparison
                k_wq = self.k_proj.weight_q.value
                k_ws = self.k_proj.weight_scale.value
                k_wq_f32 = k_wq.astype(jnp.float32)
                jax.debug.print(
                    "[ATTN_PRE] layer={lid} k_proj FP8: weight_q_shape={wqs} weight_scale_shape={wss}"
                    " weight_q_absmax={wqa} weight_scale_mean={wsm} weight_scale_absmax={wsa}"
                    " weight_block_size={wbs}",
                    lid=self.layer_id,
                    wqs=k_wq.shape,
                    wss=k_ws.shape,
                    wqa=jnp.abs(k_wq_f32).max(),
                    wsm=k_ws.mean(),
                    wsa=jnp.abs(k_ws).max(),
                    wbs=str(getattr(self.k_proj, "weight_block_size", None)),
                )
            elif hasattr(self.v_proj, "weight"):
                # BF16 path
                v_w = self.v_proj.weight.value
                jax.debug.print(
                    "[ATTN_PRE] layer={lid} v_proj BF16: weight_shape={ws} weight_absmax={wa}",
                    lid=self.layer_id,
                    ws=v_w.shape,
                    wa=jnp.abs(v_w).max(),
                )
        ...
        # Debug: check v output for layer 0 and 1
        if _dbg_attn:
            jax.debug.print(
                "[V_PROJ_OUT] layer={lid} v_absmax={va} v_mean={vm} v_std={vs}",
                lid=self.layer_id,
                va=jnp.abs(v).max(),
                vm=v.mean(),
                vs=v.std(),
        ...
        # Debug: check q/k/v shapes and positions
        _dbg_attn = self.layer_id in (0, 1, 5)
        if _dbg_attn:
            jax.debug.print(
                "[ATTN_DBG] layer={lid} q_shape={qs} k_shape={ks} v_shape={vs} positions={pos} q_absmax={qa} k_absmax={ka} v_absmax={va}",
                lid=self.layer_id,
                qs=q.shape,
                ks=k.shape,
                vs=v.shape,
        ...
        if _dbg_attn:
            ...
            if kv_fused is not None:
                if isinstance(kv_fused, tuple):
                    kv_shape_str = f"tuple({len(kv_fused)})"
                else:
                    kv_shape_str = str(kv_fused.shape)
            jax.debug.print(
                "[ATTN_DBG] layer={lid} after_attn kv_fused_type={kvs} attn_out_absmax={ao}",
                lid=self.layer_id,
                kvs=kv_shape_str,
                ao=jnp.abs(attn_output).max(),
            )
...
class MiMoMoeDecoderLayer(nnx.Module):
    ...
    def __call__(
        ...
        _dbg = self.layer_id in (0, 1, 5)
        if _dbg:
            jax.debug.print(
                "[DBG] layer={lid} pre_attn absmax={a} mean={m}",
                lid=self.layer_id,
                a=jnp.abs(hidden_states).max(),
                m=hidden_states.mean(),
            )
        ...
        if _dbg:
            # Check if attn output is constant across sequence
            attn_mean_per_pos = hidden_states.mean(axis=-1)  # [seq_len]
            attn_std_across_pos = attn_mean_per_pos.std()
            jax.debug.print(
                "[DBG] layer={lid} attn_out absmax={a} mean={m} std_across_pos={s} attn_in_absmax={ai} residual_absmax={r}",
                lid=self.layer_id,
                a=jnp.abs(hidden_states).max(),
                m=hidden_states.mean(),
                s=attn_std_across_pos,
                ai=jnp.abs(attn_in).max(),
                r=jnp.abs(residual).max(),
            )
        ...
        if _dbg:
            jax.debug.print(
                "[DBG] layer={lid} post_attn_residual absmax={r} pre_mlp absmax={h}",
                lid=self.layer_id,
                r=jnp.abs(residual).max(),
                h=jnp.abs(hidden_states).max(),
            )
        ...
        if _dbg:
            jax.debug.print(
                "[DBG] layer={lid} mlp_out absmax={m} final_hidden absmax={h}",
                lid=self.layer_id,
                m=jnp.abs(mlp_output).max(),
                h=jnp.abs(hidden_states).max(),
            )
...
class MiMoV2Model(nnx.Module):
    ...
    def __call__(
        ...
        # Debug: check pre-norm hidden states
        jax.debug.print(
            "[PRE_NORM] absmax={a} mean={m} std={s}",
            a=jnp.abs(hidden_states).max(),
            m=hidden_states.mean(),
            s=hidden_states.std(),
        )
...
class MiMoV2FlashForCausalLM(nnx.Module):
    ...
    def _create_moe_layer_mappings(self, layer_idx: int, model_config: ModelConfig) -> dict:
        ...
        has_blockwise_linear_scale = bool(
            is_quantized and getattr(quant_config, "weight_block_size", None) is not None
        )
    ...
    def __call__(
        ...
        # Debug: check final hidden states before lm_head
        jax.debug.print(
            "[FINAL_HIDDEN] absmax={a} mean={m} std={s} shape={sh}",
            a=jnp.abs(hidden_states).max(),
            m=hidden_states.mean(),
            s=hidden_states.std(),
        ...
        if hasattr(output, 'next_token_logits') and output.next_token_logits is not None:
            ...
            jax.debug.print(
                "[LOGITS] absmax={a} mean={m} std={s} argmax={am}",
                a=jnp.abs(logits).max(),
                m=logits.mean(),
                s=logits.std(),
