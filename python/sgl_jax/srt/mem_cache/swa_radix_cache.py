...
class SWARadixCache(BasePrefixCache):
    ...
    def full_lru_list_evictable_size(self) -> int:
        node = self.full_lru_list.get_lru_no_lock()
        evictable_size = 0
        while self.full_lru_list.in_list(node):
            evictable_size += len(node.value)
            node = self.full_lru_list.get_prev_no_lock(node)
        return evictable_size

    # Note: this is expensive, only use for debug
    def swa_lru_list_evictable_size(self) -> int:
        # For SWA we account by effective length (mapped entries only).
        # Walk the SWA LRU from the least-recently-used unlocked node and
        # sum the effective lengths.
        x = self.swa_lru_list.get_lru_no_lock()
        total = 0
    ...
    def _split_node(self, child: TreeNode, split_len: int) -> TreeNode:
        ...
        # Use copies to avoid future in-place changes through shared views
        new_node.value = np.array(child.value[:split_len], copy=True)
    ...
    def _insert_helper(self, node: TreeNode, key: RadixKey, value, update_kv_after_len: int) -> int:
        ...
        while len(key) > 0 and child_key in node.children:
            ...
            if update_kv_after_len < total_prefix_length + prefix_len:
                ...
                if node.swa_tombstone:
                    ...
                    self.swa_evictable_size_ += self._swa_eff_len(node.value, dp_rank=node_dp_rank)
                else:
        ...
        if len(key):
            ...
            self.swa_evictable_size_ += self._swa_eff_len(new_node.value, dp_rank=new_node_dp_rank)
