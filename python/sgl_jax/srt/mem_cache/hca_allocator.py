...
class HCAKVPoolAllocator:
    ...
    def ensure_compressed_capacity(self, req_pool_indices, seq_lens) -> None:
        ...
        active = seq_lens > 0
        req_pool_indices = req_pool_indices[active]
        seq_lens = seq_lens[active]
        if np.any((req_pool_indices < 0) | (req_pool_indices >= self.request_pool.size)):
    ...
    def page_tables(self, req_pool_indices, seq_lens):
        ...
        if np.any(seq_lens < 0):
            raise ValueError("HCA sequence lengths must be non-negative")
        active = seq_lens > 0
        active_indices = req_pool_indices[active]
        if np.any((active_indices < 0) | (active_indices >= self.request_pool.size)):
            raise ValueError("active HCA requests require allocated request slots")
        desired = np.floor_divide(seq_lens, self.token_pool.compress_ratio)
        if np.any(desired[active] > self.compressed_capacity[active_indices]):
            raise RuntimeError("ensure_compressed_capacity must run before page_tables")
        page_size = self.page_size
        window_pages = []
        window_cu = [0]
        for req_index, seq_len in zip(req_pool_indices, seq_lens):
            logical_pages = max(1, math.ceil(int(seq_len) / page_size))
            if seq_len:
                physical_pages = self.window_slots[req_index, ::page_size] // page_size
                pages = physical_pages[
                    np.arange(logical_pages, dtype=np.int32) % physical_pages.size
                ]
            else:
                pages = np.zeros((1,), np.int32)
            window_pages.extend(pages)
