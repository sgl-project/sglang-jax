...
def _set_kv_buffer(
    ...
    v_cache: jax.Array,
    """
    k_cache_ref = jax.array_ref(k_cache)
    v_cache_ref = jax.array_ref(v_cache)

    k_cache_ref.at[loc].set(k)
    v_cache_ref.at[loc].set(v)

    k_cache = jax.ref.freeze(k_cache_ref)
    v_cache = jax.ref.freeze(v_cache_ref)
