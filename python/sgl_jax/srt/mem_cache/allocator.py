...
class SWATokenToKVPoolAllocator(BaseTokenToKVPoolAllocator):
    ...
    def alloc_extend(self, prefix_lens, seq_lens, last_loc, extend_num_tokens):
        ...
        # Allocate full
        full_result = self.full_attn_allocator.alloc_extend(prefix_lens, seq_lens, last_loc, extend_num_tokens)
        if full_result is None:
            return None
        full_out_cache_loc, _ = full_result
        ...
        # Allocate SWA
        swa_result = self.swa_attn_allocator.alloc_extend(prefix_lens, seq_lens, swa_last_loc, extend_num_tokens)
        if swa_result is None:
            # Rollback full allocation
            self.full_attn_allocator.free(full_out_cache_loc)
            return None
        swa_out_cache_loc, _ = swa_result
    ...
    def alloc_decode(self, seq_lens, last_loc):
        ...
        # Allocate full
        full_result = self.full_attn_allocator.alloc_decode(seq_lens, last_loc)
        if full_result is None:
            return None
        full_out_cache_loc, _ = full_result
        ...
        # Allocate SWA
        swa_result = self.swa_attn_allocator.alloc_decode(seq_lens, swa_last_loc)
        if swa_result is None:
            # Rollback full allocation
            self.full_attn_allocator.free(full_out_cache_loc)
            return None
