...
class UnifiedRadixCache(BasePrefixCache):
    ...
    def _match_prefix_helper(
        ...
        # Host-best: deepest node reachable through the L2 (backuped) tier. With
        # HiCache disabled the host validators equal the device ones, so the host
        # fields collapse onto the device ones and host_hit_length is 0.
        best_host_node = node
        best_host_tokens = 0
        # Running token count along the walk (device coverage is always a
        # contiguous prefix from root, so device-best stops advancing at the
        # first evicted node while host-best may continue).
        cur_tokens = 0
        # Once the device chain is broken by a tombstone (evicted-but-backuped)
        # node, device-best must NOT advance to a deeper still-resident node:
        # its ancestors would no longer all be on device, breaking the
        # contiguous-prefix invariant that inc_lock_ref relies on. Partial
        # load_back can leave such gaps, so we enforce contiguity here.
        device_broken = False
