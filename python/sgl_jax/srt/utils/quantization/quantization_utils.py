...
def apply_linear_quantization(
    ...
    for rule in linear_rules:
        ...
        act_tile_size = rule.get("activation_tile_size", rule.get("tile_size"))
        if act_tile_size is not None:
            act_tile_size = int(act_tile_size)
            if act_tile_size <= 0:
                raise ValueError(f"tile_size must be > 0, got {act_tile_size}")

        act_channelwise_axes = rule.get(
            "activation_channelwise_axes",
            rule.get("channelwise_axes"),
        )
        if act_channelwise_axes is not None:
            act_channelwise_axes = tuple(int(x) for x in act_channelwise_axes)

        act_calibration_method = rule.get(
            "activation_calibration_method",
            rule.get("calibration_method"),
        )
        ...
        compiled_rules.append(
            {
                ...
                "activation_qdq_tile_size": act_tile_size,
                "activation_qdq_channelwise_axes": act_channelwise_axes,
                "activation_qdq_calibration_method": act_calibration_method,
            }
        )
                ...
                if isinstance(attr_value, LinearBase):
                    ...
                    if rule is not None:
                        ...
                        quantized_linear = QuantizedLinear.from_linear(
                            ...
                            activation_qdq_tile_size=rule["activation_qdq_tile_size"],
                            activation_qdq_channelwise_axes=rule["activation_qdq_channelwise_axes"],
                            activation_qdq_calibration_method=rule[
                                "activation_qdq_calibration_method"
                            ],
                        )
