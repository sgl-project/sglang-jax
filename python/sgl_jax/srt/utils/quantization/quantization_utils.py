...
def calculate_model_weights_memory(model: nnx.Module) -> dict[str, float]:
    """
    Calculate the total memory used by model weights.
    ...
    def _calculate_recursive(obj, visited=None):
        ...
        # Check if this is an nnx.Param
        if isinstance(obj, nnx.Param):
            value = obj.value
            if hasattr(value, "shape") and hasattr(value, "dtype"):
                param_size = int(np.prod(value.shape))
                dtype_size = jnp.dtype(value.dtype).itemsize
                total_bytes += param_size * dtype_size
            return
        ...
        if hasattr(obj, "__dict__"):
            for attr_name, attr_value in obj.__dict__.items():
                if attr_name.startswith("_"):
                    continue
                if isinstance(attr_value, (nnx.Module, nnx.Param)):
                    _calculate_recursive(attr_value, visited)
                elif isinstance(attr_value, (list, tuple)):
                    for item in attr_value:
                        if isinstance(item, (nnx.Module, nnx.Param)):
                            _calculate_recursive(item, visited)
                elif isinstance(attr_value, dict):
                    for v in attr_value.values():
                        if isinstance(v, (nnx.Module, nnx.Param)):
                            _calculate_recursive(v, visited)

    _calculate_recursive(model)
...
def log_quantization_memory_savings(
    memory_before: dict[str, float],
    memory_after: dict[str, float],
) -> None:
    """
    Log the memory savings from quantization.

    Args:
        memory_before: Memory stats before quantization (from calculate_model_weights_memory)
        memory_after: Memory stats after quantization (from calculate_model_weights_memory)
    """
    bytes_before = memory_before["total_bytes"]
    bytes_after = memory_after["total_bytes"]
    bytes_saved = bytes_before - bytes_after
    savings_pct = (bytes_saved / bytes_before * 100) if bytes_before > 0 else 0

    logger.info(
        "Quantization Memory Savings: Before: %.2f GB, After: %.2f GB, Saved: %.2f GB (%.1f%%)",
        memory_before["total_gb"],
        memory_after["total_gb"],
        bytes_saved / _GB,
        savings_pct,
    )
