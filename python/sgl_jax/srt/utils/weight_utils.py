...
def create_epmoe_weights_mapping(
    prefix: str,
    target_prefix: str,
    num_experts: int,
    expert_type_map: dict[str, str] | None = None,
    moe_path: str = "mlp",
    source_expert_pattern: str = "experts.{i}",
    transpose: bool = False,
    concat_axis_map: dict[str, int] | None = None,
) -> dict[str, WeightMapping]:
    """
    Generate a unified mapping dictionary for EPMoE layer expert weights.

    The sharding strategy is aligned with the PartitionSpec defined in EPMoE:
    - wi_0/wi_1 (Input projections): P("expert", "tensor", None)
    - wo (Output projection): P("expert", None, "tensor")

    Args:
        prefix: Source weight path prefix (e.g., "model.layers.0")
        target_prefix: Target model path prefix (e.g., "model.layers.0")
        num_experts: Number of experts in the MoE layer
        expert_type_map: Mapping from HuggingFace weight name to EPMoE internal variable name.
            Default: {"gate_proj": "wi_0", "up_proj": "wi_1", "down_proj": "wo"}
        moe_path: Path to the MoE module within a layer (e.g., "mlp" or "block_sparse_moe")
        source_expert_pattern: Pattern for expert indexing in the source weight file.
            Default: "experts.{i}" which produces "experts.0", "experts.1", etc.
        transpose: Whether to transpose the weights during loading
        concat_axis_map: Map from source weight name to its concatenation axis

    Returns:
        Dictionary of weight mappings with __MOE_EXPERTS__ prefix keys
    """
    ...
    for source_name, target_name in expert_type_map.items():
        ...
        # EPMoE sharding: wo uses ("expert", None, "tensor"), others use ("expert", "tensor", None)
        sharding = ("expert", None, "tensor") if target_name == "wo" else ("expert", "tensor", None)

        concat_axis = concat_axis_map.get(source_name)
...
def create_fused_moe_weights_mapping(
    prefix: str,
    target_prefix: str,
    num_experts: int,
    expert_type_map: dict[str, str] | None = None,
    moe_path: str = "mlp",
    source_expert_pattern: str = "experts.{i}",
    sharding: tuple[str | None, ...] = ("tensor", None, None),
    concat_axis_map: dict[str, int] | None = None,
) -> dict[str, WeightMapping]:
    """
    Generate a unified mapping dictionary for Fused MoE layer expert weights.

    The default sharding strategy is ("tensor", None, None) for all weights.

    Args:
        prefix: Source weight path prefix (e.g., "model.layers.0")
        target_prefix: Target model path prefix (e.g., "model.layers.0")
        num_experts: Number of experts in the MoE layer
        expert_type_map: Mapping from HuggingFace weight name to Fused MoE internal variable name.
            Default: {"gate_proj": "w1", "up_proj": "w3", "down_proj": "w2"}
        moe_path: Path to the MoE module within a layer (e.g., "mlp" or "block_sparse_moe")
        source_expert_pattern: Pattern for expert indexing in the source weight file.
            Default: "experts.{i}" which produces "experts.0", "experts.1", etc.
        sharding: Sharding specification for all weights. Default: ("tensor", None, None)
        concat_axis_map: Map from source weight name to its concatenation axis.
            Commonly used for TP-split weights (e.g., Grok).
