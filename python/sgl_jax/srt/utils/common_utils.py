...
@lru_cache(maxsize=1)
def is_hpu() -> bool:
    """Check if HPU (Habana) devices are available."""
    try:
        devices = jax.devices()
        return any(device.platform == "hpu" for device in devices)
    except Exception:
        return False
...
@lru_cache(maxsize=1)
def is_xpu() -> bool:
    """Check if XPU (Intel) devices are available."""
    try:
        devices = jax.devices()
        return any(device.platform == "xpu" for device in devices)
    except Exception:
        return False
...
@lru_cache(maxsize=1)
def is_npu() -> bool:
    """Check if NPU (Huawei) devices are available."""
    try:
        devices = jax.devices()
