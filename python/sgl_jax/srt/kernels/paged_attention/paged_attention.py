...
#
# ==============================================================================
# Modifications by Yanko (@Yanko-7) for sgl-jax:
# - Implemented SPMD distributed inference via `jax.shard_map`.
# - Added `xai_temperature_len` for length-based temperature scaling.
# - Supported `sm_scale` for attention logit scaling.
# ==============================================================================
...
def paged_attention_kernel(
    ...
    *residual_refs: Any,  # Residual outputs: [block_h,], [block_h,]
    ...
    def _compute(start_page_idx, end_page_idx, o, m_i, l_i):
        ...
        # [NEW] XAI Scaling Logic
        xai_regulator = 1.0
        if xai_temperature_len is not None and xai_temperature_len > 0:
            current_seq_len = lengths_ref[0]
            xai_scale = 1.0 / jnp.log2(float(xai_temperature_len))
            log_pos = jnp.log2(jnp.maximum(current_seq_len, 1.0))
            temp_factor = log_pos * xai_scale
            xai_regulator = jnp.where(current_seq_len > xai_temperature_len, temp_factor, 1.0)

        sm_scale_adjusted = sm_scale * xai_regulator
        ...
        def body(start_k, carry):
            ...
            block_tables_slice = pl.ds(start_k * pages_per_compute_block, pages_per_compute_block)
            ...
            if k_scales_pages_ref is not None:
                ...
                k = k.astype(q.dtype)
            uncapped_logits = pl.dot(q, k.T) * sm_scale_adjusted  # [block_h, block_k]
            if k_scales_pages_ref is not None:
...
def paged_attention_unbatched(
    ...
    block_table: jax.Array,  #  [pages_per_sequence]
    length: jax.Array | None,  #  [1]
    ...
    *,
    ...
    def _local_impl(
        ...
        num_q_heads, head_dim = q_loc.shape
        num_kv_heads, total_num_pages, page_size, _ = k_pages_loc.shape
        # pages_per_sequence has been padded to a power of 2.
        pages_per_sequence = block_tables_loc.shape[0]

        # global is valid, the Local shard is usually also valid.
        assert (
        ...
        _pages_per_compute_block = min(pages_per_partition, pages_per_compute_block)

        assert (
            pages_per_partition % _pages_per_compute_block == 0
        ), f"{pages_per_partition=} must de divisible by {_pages_per_compute_block=}."

        block_tables_reshaped = block_tables_loc.reshape(k_splits, pages_per_sequence // k_splits)

        q_heads_per_kv_head = num_q_heads // num_kv_heads
        q_reshaped = q_loc.reshape(num_kv_heads, q_heads_per_kv_head, head_dim)

        if q_heads_per_kv_head % block_h:
            q_reshaped = jnp.pad(q_reshaped, ((0, 0), (0, -q_heads_per_kv_head % block_h), (0, 0)))
        ...
        if k_scales_loc is not None:
            k_scales_spec = pl.BlockSpec(
                (None, total_num_pages, page_size), lambda h, i, k: (h, 0, 0)
            )
        else:
            k_scales_spec = None
        if v_scales_loc is not None:
            v_scales_spec = pl.BlockSpec(
                (None, total_num_pages, page_size), lambda h, i, k: (h, 0, 0)
            )
        else:
            v_scales_spec = None
        ...
        partial_out, partial_exp_sums, partial_max_logits = pl.pallas_call(
            ...
            in_specs=[
                ...
                pl.BlockSpec((None, pages_per_partition), lambda h, i, k: (k, 0)),  # block_tables
                pl.BlockSpec((1,), lambda h, i, k: (0,)),  # lengths
            ],
            out_specs=[
            ...
            out_shape=[
                jax.ShapeDtypeStruct((k_splits, *q_reshaped.shape), dtype=q_loc.dtype),  # o
            ...
            name=f"paged_attention_{block_h=}_{_pages_per_compute_block=}",
        )(
            q_reshaped,  # [num_kv_heads, q_heads_per_kv_head, head_dim]
            k_pages_loc,  # [num_kv_heads, total_num_pages, page_size, head_dim]
            k_scales_loc,
            v_pages_loc,
            v_scales_loc,
            block_tables_reshaped,  # [k_splits, pages_per_partition]
            lengths_loc,  # [1]
        )
    ...
    sharded_args = [q, k_pages, v_pages, block_table, length]
    in_specs = [
        P(sharding_axis, None),  # q: [num_q_heads, head_dim] -> [local_q, head_dim]
        P(sharding_axis, None, None, None),  # k: [num_kv_heads, ...] -> [local_kv, ...]
        P(sharding_axis, None, None, None),  # v
        P(None),  # block_tables
        P(None),  # length
    ]

    if k_scales_pages is not None:
        sharded_args.append(k_scales_pages)
        in_specs.append(P(sharding_axis, None, None))
    else:
        sharded_args.append(None)
        in_specs.append(P(None))

    if v_scales_pages is not None:
        sharded_args.append(v_scales_pages)
        in_specs.append(P(sharding_axis, None, None))
    else:
        sharded_args.append(None)
        in_specs.append(P(None))
    ...
    out_specs = P(sharding_axis, None)

    op = jax.shard_map(
        _local_impl, mesh=mesh, in_specs=tuple(in_specs), out_specs=out_specs, check_vma=False
    )

    return op(*sharded_args)
...
def paged_attention(
    ...
    *,
    ...
    xai_temperature_len: float | None = None,
    attn_logits_soft_cap: float | None = None,
    mesh: Mesh | None = None,  # new
    sharding_axis: str = "tensor",  # new
) -> jax.Array:
    ...
    # global_q / global_kv == local_q / local_kv
    ...
    # q, block_tables, lengths are batched (0)
    # k_pages, v_pages, scales are unbatched (None)
...
@functools.partial(jax.jit, static_argnames=["mask_value", "attn_logits_soft_cap"])
def paged_attention_reference(
    ...
    *,
    ...
    q_reshaped = q.reshape(batch_size, num_kv_heads, q_heads_per_kv_head, head_dim)
    k_transposed = jnp.swapaxes(k, 1, 2)  # [batch_size, num_kv_heads, kv_seq_len, head_dim]
    v_transposed = jnp.swapaxes(v, 1, 2)  # [batch_size, num_kv_heads, kv_seq_len, head_dim]

    uncapped_logits = (
        jnp.einsum(
            "bkgd,bksd->bkgs", q_reshaped, k_transposed, preferred_element_type=jnp.float32
        ).astype(jnp.float32)
        * sm_scale
    )
    ...
    o = jnp.einsum(
        "bkgs,bksd->bkgd",
        weights,
        v_transposed.astype(jnp.float32),
