            ...
            for start_n in range(0, cfgs.tiles.tile_n, mxu_size):
                ...
                for b_id in range(num_quant_blocks_per_tile_k):
                    ...
                    if tiled_rhs_ref.scale is not None:
                        rhs_scale_slice = tiled_rhs_ref.scale[..., b_id, :, start_n:end_n]
...
def kernel_main(
    ...
    *,
    ...
    # kernel.  [G, K, N] -> [G, K//packing, N] uint32
...
def calculate_tiling(
    ...
    def _is_tile_k_quant_block_compatible(tk: int) -> bool:
        return tk % rhs_cfgs.quant_block_size == 0 or rhs_cfgs.quant_block_size % tk == 0
