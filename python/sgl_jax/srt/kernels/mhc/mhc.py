...
def mhc_post_fused(
    ...
    *,
    ...
    if residual_streams.ndim < 3:
        raise ValueError(f"residual_streams must be [..., hc, d], got {residual_streams.shape}")
    outer_shape = residual_streams.shape[:-2]
    hc, d = residual_streams.shape[-2:]
    expected_x = (*outer_shape, d)
    expected_post = (*outer_shape, hc)
    expected_comb = (*outer_shape, hc, hc)
    if x.shape != expected_x:
        raise ValueError(f"x must be [..., d]={expected_x}, got {x.shape}")
    if post.shape != expected_post:
        raise ValueError(f"post must be [..., hc]={expected_post}, got {post.shape}")
    if comb.shape != expected_comb:
        raise ValueError(f"comb must be [..., hc, hc]={expected_comb}, got {comb.shape}")

    # Flattening leading dimensions is a TPU layout bitcast.
    x = x.reshape(-1, d)
    residual_streams = residual_streams.reshape(-1, hc, d)
    ...
    if backend == "xla":
        ...
        return y.reshape(*outer_shape, hc, d)
    ...
    return y[:n].reshape(*outer_shape, hc, d)
