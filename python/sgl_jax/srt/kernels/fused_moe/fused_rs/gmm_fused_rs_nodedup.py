...
# Packed routing indices use one int32 per routed row. Keep the fast scalar-
# prefetch path while it fits below the TPU SMEM ceiling; larger prefill shapes
# stage one tile at a time from HBM into a double-buffered VMEM scratch.
_FUSED_RS_SMEM_INDEX_LIMIT_ROWS = 240_000

# Tuning-only override. It is read while the jitted call is being traced; the
# benchmark clears JAX's trace caches between candidates. Production callers
...
# GLM-5.2 routing (Falcon exp-36nk50wmtt, exp-ag14zx1qrc, exp-4vr7cnvusj),
# 2026-08-18.
_GLM52_V7X_EP16_FP8_BLOCK_SIZES: dict[int, FusedRsBlockConfig] = {
    256: (128, 6144, 2048, 2048, 6144, 1, 1),
    512: (128, 6144, 2048, 2048, 6144, 1, 1),
    1024: (128, 6144, 2048, 2048, 6144, 1, 1),
    2048: (128, 6144, 2048, 2048, 6144, 1, 1),
    4096: (128, 6144, 2048, 2048, 6144, 1, 1),
    8192: (128, 6144, 2048, 2048, 6144, 1, 1),
    16384: (128, 6144, 2048, 2048, 6144, 1, 1),
    32768: (128, 6144, 2048, 2048, 6144, 1, 1),
    65536: (256, 6144, 2048, 2048, 3072, 1, 1),
    131072: (256, 6144, 2048, 2048, 3072, 1, 1),
}

# Strict production-shape tuning: 16 physical TPU v7x chips / 32 devices,
# data1xtensor32, 65,536 tokens, Gaussian TopK8 routing (seed 42). The key is
# the routed M dimension (65,536 * 8). This entry remains deliberately narrow:
...
def _is_tpu_v7x_ep_size(ep_size: int | None, expected_ep_size: int) -> bool:
    """Check the tuned hardware without importing serving-side utilities."""
    ...
    try:
        ...
        return False
    return kind in ("TPU7x", "TPU v7")


def set_fused_rs_block_sizes_override(
    block_sizes: FusedRsBlockConfig | None,
) -> None:
    """Override block sizes for one tuning candidate.

    Call ``jax.clear_caches()`` after changing the override and before tracing
    another candidate. This hook is intentionally process-local and is not used
    by serving code.
    """
    global _FUSED_RS_BLOCK_SIZES_OVERRIDE
    if block_sizes is not None:
        if len(block_sizes) != 7 or any(value <= 0 for value in block_sizes):
            raise ValueError(
                "fused_rs block sizes must contain seven positive integers: "
                "tile_m,tile_k1,tile_n1,tile_k2,tile_n2,num_w1_bufs,num_w2_bufs"
            )
        block_sizes = tuple(int(value) for value in block_sizes)
    _FUSED_RS_BLOCK_SIZES_OVERRIDE = block_sizes
...
def get_fused_rs_tuned_block_sizes(
    ...
    if _FUSED_RS_BLOCK_SIZES_OVERRIDE is not None:
        result = _FUSED_RS_BLOCK_SIZES_OVERRIDE
    else:
        is_glm52_v7x_ep16_fp8 = (
            _is_tpu_v7x_ep_size(ep_size, 16)
            and k1 == 6144
            and n1 == 4096
            and k2 == 2048
            and n2 == 6144
            and num_current_groups == 16
            and lhs_dtype == jnp.bfloat16
            and rhs_dtype == jnp.float8_e4m3fn
            and rhs_quant_block_size == 128
            and fuse_act == "silu"
            and not fp8_direct_write
        )
        is_glm52_v7x_ep32_per_channel_fp8 = (
            _is_tpu_v7x_ep_size(ep_size, 32)
            and k1 == 6144
            and n1 == 4096
            and k2 == 2048
            and n2 == 6144
            and num_current_groups == 8
            and lhs_dtype == jnp.bfloat16
            and rhs_dtype == jnp.float8_e4m3fn
            and rhs_quant_block_size == 6144
            and fuse_act == "silu"
            and not fp8_direct_write
        )
...
def _build_packed_index_tile_metadata(
    ...
    *,
    ...
    first_local_group = group_offset[0]
    is_local = jnp.logical_and(
        group_ids >= first_local_group,
        group_ids < first_local_group + num_local_groups,
    )
    local_offsets = group_starts % size_lhs_sublane
    num_tiles = jnp.where(
        jnp.logical_and(is_local, group_sizes > 0),
        pl.cdiv(group_sizes + local_offsets, tile_m),
        0,
    ).astype(jnp.int32)
    tile_ends = jnp.cumsum(num_tiles, dtype=jnp.int32)

    gm_ids = jnp.arange(max_num_gm, dtype=jnp.int32)
    gm_group_ids = jnp.searchsorted(tile_ends, gm_ids, side="right")
    gm_group_ids = jnp.minimum(gm_group_ids, group_sizes.shape[0] - 1)
    tile_starts_for_group = tile_ends - num_tiles
    tile_ordinals = gm_ids - tile_starts_for_group[gm_group_ids]

    group_start = group_starts[gm_group_ids]
    first_tile_capacity = tile_m - (group_start % size_lhs_sublane)
    tile_start = jnp.where(
        tile_ordinals == 0,
        group_start,
        group_start + first_tile_capacity + (tile_ordinals - 1) * tile_m,
    )
    num_rows = jnp.clip(
        group_ends[gm_group_ids] - tile_start,
        min=0,
        max=tile_m,
    )
...
def _select_fused_rs_block_sizes(
    *,
    ...
    # has multiple weight steps; a single buffer is safe only for a full-width,
    # single-step GMM.  This also rejects unsafe tuning overrides instead of
    # silently accepting a configuration that constant weights can mask.
    total_w1_steps = (out_n1 // tile_n1) * (size_k1 // tile_k1)
    total_w2_steps = (size_n2 // tile_n2) * (padded_k2 // tile_k2)
    if total_w1_steps > 1 and num_w1_bufs < 2:
        raise ValueError(
            "fused_rs requires at least two W1 buffers for multi-step GMM1; "
            f"got steps={total_w1_steps}, buffers={num_w1_bufs}"
        )
    if total_w2_steps > 1 and num_w2_bufs < 2:
        raise ValueError(
            "fused_rs requires at least two W2 buffers for multi-step GMM2; "
            f"got steps={total_w2_steps}, buffers={num_w2_bufs}"
        )
    ...
    global _FUSED_RS_LAST_SELECTED_BLOCK_SIZES
    _FUSED_RS_LAST_SELECTED_BLOCK_SIZES = (
        tile_m,
        tile_k1,
        tile_n1,
        tile_k2,
...
def kernel_main_fused_rs(
    ...
    *,
    ...
    indices_in_hbm: bool = False,
    fp8_direct_write: bool = False,
    separate_gate_up: bool = False,
):
    ...
    # faster peer can remotely write a valid row while this chip's zero-fill is
    # still in flight, after which the zero DMA overwrites the remote result.
    # Finish zeroing on every chip and synchronize the mesh before any GMM tile
    # can issue local or remote output writes.  Sparse padded batches make the
    # race especially visible because per-chip expert work is imbalanced.
    ...
    if cfgs1.rhs_cfgs.quant_dtype is not None:
        ...
        w3_packed = w3_ref.bitcast(jnp.uint32) if separate_gate_up else w1_packed
    else:
        w1_packed = w1_ref
        w3_packed = w3_ref if separate_gate_up else w1_packed
    ...
    def start_w1_dma(buf_id, expert_id, n_id, k_id=0):
        if act_fn and separate_gate_up:
            # Production layers retain gate/up as distinct checkpoint weights
            # so fused-v2 decode and fused-RS prefill can share parameters. Join
            # only the current VMEM tile instead of materializing an HBM w13.
            _offset = n_id * tile_n1
            pltpu.make_async_copy(
                src_ref=w1_packed.at[
                    expert_id,
                    pl.ds(k_id * pk1, pk1),
                    pl.ds(_offset, tile_n1),
                ],
                dst_ref=w1_buf_ref.at[buf_id, :, :tile_n1],
                sem=w1_sem_ref.at[buf_id],
            ).start()
            pltpu.make_async_copy(
                src_ref=w3_packed.at[
                    expert_id,
                    pl.ds(k_id * pk1, pk1),
                    pl.ds(_offset, tile_n1),
                ],
    ...
    @jax.named_scope("start_lhs_indices_dma")
    def start_lhs_indices_dma(gm_id, buf_id):
        pltpu.make_async_copy(
            src_ref=lhs_indices_hbm_ref.at[gm_id],
            dst_ref=lhs_indices_2x_vmem_ref.at[buf_id],
            sem=lhs_indices_sem_ref.at[buf_id],
        ).start()

    @jax.named_scope("wait_lhs_indices_dma")
    def wait_lhs_indices_dma(buf_id):
        pltpu.make_async_copy(
            src_ref=lhs_indices_2x_vmem_ref.at[buf_id],
            dst_ref=lhs_indices_2x_vmem_ref.at[buf_id],
            sem=lhs_indices_sem_ref.at[buf_id],
        ).wait()
        pltpu.async_copy(
            src_ref=lhs_indices_2x_vmem_ref.at[buf_id],
            dst_ref=lhs_indices_2x_ref.at[buf_id],
            sem=lhs_indices_sem_ref.at[buf_id],
        ).wait()
    ...
    if indices_in_hbm:
        ...
        @pl.when(local_num_gm > 0)
        def _():
            start_lhs_indices_dma(jnp.int32(0), jnp.int32(0))
    ...
    def gm_loop_body(gm_id, _):
        ...
        def _():
            ...
            if indices_in_hbm:
                ...
                @pl.when(gm_id == 0)
                def _():
                    wait_lhs_indices_dma(jnp.int32(0))

                current_lhs_indices_ref = lhs_indices_2x_ref.at[sem_id, 0]
            else:
                current_lhs_indices_ref = lhs_indices_ref
            ...
            def _():
                next_lhs_indices_ref = lhs_indices_ref
                if indices_in_hbm:
                    next_buf_id = 1 - sem_id
                    start_lhs_indices_dma(gm_id + 1, next_buf_id)
                    wait_lhs_indices_dma(next_buf_id)
                    next_lhs_indices_ref = lhs_indices_2x_ref.at[next_buf_id, 0]
                next_gather_meta = prepare_gather_tile_metadata(
                    gm_id + 1,
                    metadata_ref,
                    tile_m=tile_m,
                    size_m=fused_dims.size_m,
                    lhs_indices_ref=next_lhs_indices_ref,
                    pack_indices=pack_indices,
                    gather_divisor=_gather_divisor,
                    indices_are_tile_local=indices_in_hbm,
                )
...
def gmm_v2_fused_rs(
    ...
    *,
    ...
    if separate_gate_up:
        if not act_fn:
            raise ValueError("separate w1/w3 requires a fused gated activation")
        assert w3 is not None
        assert w3.shape == w1.shape
        assert w3.dtype == w1.dtype
        size_n1 = gate_n1 * 2
    else:
        size_n1 = gate_n1
    ...
    if is_quantized:
        ...
        if w1_scale.shape[1] < total_scale_blocks1:
            ...
            if separate_gate_up:
                w3_scale = jnp.concatenate(
                    [
                        w3_scale,
                        jnp.repeat(
                            w3_scale[:, -1:, :, :],
                            total_scale_blocks1 - w3_scale.shape[1],
                            axis=1,
                        ),
                    ],
                    axis=1,
    ...
    lhs1_quant_block_size = (
        size_k1
        if rhs1_quant_block_size == size_k1
        else min(256 if rhs1_quant_block_size < 512 else 512, size_k1)
    )
    lhs2_quant_block_size = (
        size_k2
