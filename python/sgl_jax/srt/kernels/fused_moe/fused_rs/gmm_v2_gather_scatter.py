    ...
    *,
    ...
    def _matmul(is_first_k_step: bool, is_last_k_step: bool):
        ...
        if cfgs.rhs_cfgs.quant_dtype is None:
            ...
            # activation block.
            rhs_scale_is_tile_constant = (
                apply_rhs_scale_after_matmul and rhs_qbs >= cfgs.tiles.tile_k
            )
            ...
            for start_n in range(0, rhs_tile_n, mxu_size):
                ...
                for start_k in range(0, cfgs.tiles.tile_k, q_block_size):
                    ...
                    if fp8_activation_quant:
                        if cfgs.lhs_cfgs.prequantized:
                            # The outer fused-RS pipeline gathers prequantized
                            # FP8 rows into a row-zero-aligned local tile. Convert
                            # through registers and requantize per row, composing
                            # the local requant scale with the wire scale. This
                            # keeps the communicated tensor FP8 and avoids
                            # materializing the full gathered tensor as BF16 in
                            # HBM.
                ...
                if rhs_scale_is_tile_constant:
                    rhs_scale_slice = _get_scale_slice(0, start_n, end_n)
                    acc_n *= rhs_scale_slice.astype(acc_ref.dtype)
...
def prepare_gather_tile_metadata(
    ...
    *,
    ...
    indices_are_tile_local: bool = False,
) -> GatherMetadata:
    ...
    for i in range(tile_m):
        if indices_are_tile_local:
            m_idx = jnp.minimum(i, jnp.maximum(num_rows - 1, 0))
        else:
