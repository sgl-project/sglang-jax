...
t0 = time.time()
def log(m): print(f"[{time.time()-t0:.1f}s][p{jax.process_index()}] {m}", flush=True)
...
w1q, s1 = q_pc(k1, (d, se_inter)); w3q, s3 = q_pc(k2, (d, se_inter)); w2q, s2 = q_pc(k3, (se_inter, d))
w1 = repl_put(w1q); w3 = repl_put(w3q); w2 = repl_put(w2q)
s1 = repl_put(s1); s3 = repl_put(s3); s2 = repl_put(s2)

tok_local = ntok // nd
toks = jax.make_array_from_single_device_arrays(
    (ntok, d), shard,
    [jax.device_put((jax.random.normal(jax.random.fold_in(kt, i), (tok_local, d), jnp.bfloat16)), dv)
     for i, dv in enumerate(jax.local_devices())])

@jax.jit
@jax.shard_map(mesh=mesh, in_specs=(P("x"), P(), P(), P(), P(), P(), P()), out_specs=P("x"), check_vma=False)
...
def run(): return body(toks, w1, w3, w2, s1, s3, s2)
for _ in range(warmup): jax.block_until_ready(run())
ts = []
for _ in range(iters):
    a = time.monotonic(); jax.block_until_ready(run()); ts.append((time.monotonic() - a) * 1e3)
if jax.process_index() == 0:
    per = np.mean(ts) / REPS
    log(f"external SE MLP fp8: {per*1000:.1f} us/call  (tok_local={tok_local}, d={d}, se_inter={se_inter}, REPS={REPS}, wall_total={np.mean(ts):.2f}ms)")
log("done")
