        ...
        def _all_reduce_metadata(
            ...
            if num_devices > 0 and (num_devices & (num_devices - 1)) == 0:
                ...
                for round_id in range(rounds):
                    # Round synchronization is required because `recv_sem` is an
                    # accumulating counter. Without per-round alignment, a faster
                    # rank can signal a later round early and let a slower rank
                    # pass wait on the wrong round.
                    sync_barrier()
