...
def ref_moe(
    ...
    *,
    ...
    for i in range(n_tokens):
        ...
        experts_act = jnp.concatenate(tok_expert_act, axis=0)  # [top_k, d_model]
    ...
    def get_top_k(input, top_k, renormalize_topk_logits):
        ...
        for k_id in range(top_k):
            ...
            top_k_logits = jnp.broadcast_to(
                jnp.max(input, axis=1, keepdims=True), (input.shape[0], 128)
            ).astype(input.dtype)
            if renormalize_topk_logits:
                top_k_logits_sum += top_k_logits
            top_k_logits_lst.append(top_k_logits)
            # TODO(jevinjiang): support bf16 argmax in Mosaic
            top_k_indices = jnp.broadcast_to(jnp.argmax(input, axis=1, keepdims=True), input.shape)
        ...
        if renormalize_topk_logits:
            for k_id in range(top_k):
                top_k_logits_lst[k_id] = top_k_logits_lst[k_id] / top_k_logits_sum
    ...
    *,
    ...
    if subc_quant_wsz is not None:
        if subc_quant_wsz % 256 != 0:
            raise NotImplementedError("Sub-quantized window is not aligned to 256.")
    ...
    if hidden_size != actual_hidden_size or intermediate_size != actual_intermediate_size:
        ...
        if w1_scale is not None:
            w1_scale = jnp.pad(
                ...
                (
                    (0, 0),
                    (0, 0),
                    (0, cdiv(hidden_size, subc_quant_wsz) - w1_scale.shape[-3]),
                    (0, 0),
        ...
        if w2_scale is not None:
            w2_scale = jnp.pad(
                ...
                (
                    (0, 0),
                    (0, cdiv(intermediate_size, subc_quant_wsz) - w2_scale.shape[-3]),
                    (0, 0),
