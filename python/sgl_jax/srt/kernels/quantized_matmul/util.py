...
from sgl_jax.srt.kernels.quantized_matmul.tuned_block_sizes import TunedValue
...
def get_vmem_limit(
    ...
    x_size = (
        ...
        * in_block_size
        * (
            dtypes.bit_width(x_dtype)
            if hasattr(dtypes, "bit_width")
            else dtypes.itemsize_bits(x_dtype)
        )
    )
    x_abs_max_size = batch_block_size * (
        dtypes.bit_width(scale_dtype)
        if hasattr(dtypes, "bit_width")
        else dtypes.itemsize_bits(scale_dtype)
    )
    w_q_size = (
        out_block_size
        * in_block_size
        * (
            dtypes.bit_width(w_q_dtype)
            if hasattr(dtypes, "bit_width")
            else dtypes.itemsize_bits(w_q_dtype)
        )
    )
    w_scale_size = out_block_size * (
        dtypes.bit_width(scale_dtype)
        if hasattr(dtypes, "bit_width")
        else dtypes.itemsize_bits(scale_dtype)
    )
    out_size = (
        ...
        * out_block_size
        * (
            dtypes.bit_width(out_dtype)
            if hasattr(dtypes, "bit_width")
            else dtypes.itemsize_bits(out_dtype)
        )
    )
    ...
    acc_size = (
        ...
        * out_block_size
        * (
            dtypes.bit_width(acc_dtype)
            if hasattr(dtypes, "bit_width")
            else dtypes.itemsize_bits(acc_dtype)
        )
    )
    x_q_size = (
        batch_block_size
        * in_block_size
        * (
            dtypes.bit_width(x_q_dtype)
            if hasattr(dtypes, "bit_width")
            else dtypes.itemsize_bits(x_q_dtype)
        )
    )
    x_scale_size = batch_block_size * (
        dtypes.bit_width(scale_dtype)
        if hasattr(dtypes, "bit_width")
        else dtypes.itemsize_bits(scale_dtype)
    )
...
def validate_inputs(
    ...
    if x.dtype != x_q_dtype and jnp.issubdtype(x_q_dtype, jnp.integer) != jnp.issubdtype(
