    ...
    *,
    ...
    def flash_attention_step1_qk_softmax(
        ...
        *,
        ...
        q_span = (
            processed_q_len + lax.broadcasted_iota(jnp.int32, s.shape, 0) // num_q_heads_per_kv_head
        ).astype(int_ty)
        ...
        mask = jnp.logical_and(q_span < effective_kv_len, k_span < effective_kv_len)
...
def static_validate_inputs(
    ...
    *,
    ...
    if kv_cache.shape != expected_kv_cache_shape:
        raise ValueError(f"Expected {kv_cache.shape=} to be equal to {expected_kv_cache_shape=}")
