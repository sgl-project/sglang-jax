...
def fused_moe_kernel(
    ...
    for k in range(0, tl.cdiv(K, BLOCK_SIZE_K)):
        ...
        if use_int8_w8a16:
            ...
            accumulator = tl.dot(a, b, input_precision="ieee", acc=accumulator)
        # Advance the ptrs to the next K block.
        a_ptrs += BLOCK_SIZE_K * stride_ak
        b_ptrs += BLOCK_SIZE_K * stride_bk
...
def invoke_fused_moe_kernel(
    A: torch.Tensor,  # hidden_states
    B: torch.Tensor,  # w1
    bias: Optional[torch.Tensor],  # b1
    C: torch.Tensor,  # intermediate_cache
    ...
    print(f"{use_fp8_w8a8=}")
    print(f"{use_int8_w8a8=}")
    print(f"{use_int8_w8a16=}")
    print(f"{use_int4_w4a16=}")
    print(f"{per_channel_quant=}")
    print(f"{block_shape=}")
    print(f"{compute_type=}")
    print(f"{mul_routed_weight=}")
    print(f"{bias=}")
    print(f"{B_zp=}")
